...
export const courseManifest = {
    ...
    "lessons": [
      {
        ...
        "theory": "## Event-Driven Design\nEvery automation relies on an event-driven architecture. A `Trigger` is the event that starts the workflow (e.g., 'New Email Received'), while an `Action` is the task the workflow performs (e.g., 'Draft a Reply').",
        "instructions": "## Task: Identify the Trigger\nIn a workflow that says: 'When a new row is added to Google Sheets, send a Slack message', what is the trigger?",
        "starterCode": "# Options: Send Slack message, New row added, Google Sheets API\ntrigger = '___'",
        "solution": "# Options: Send Slack message, New row added, Google Sheets API\ntrigger = 'New row added'",
        "hint": "The trigger is what starts it: New row added",
        "rubric": "Identifies 'New row added'."
      },
      {
        "title": "Stateful vs Stateless",
        "theory": "## Remembering Context\nA stateless automation forgets everything once it finishes running. A stateful automation updates a central database (like a CRM) so the next time it runs, it knows what happened previously.",
        "instructions": "## Task: Concept Check\nIf your automation needs to check if a customer was already emailed last week, does it need to be stateful or stateless?",
        "starterCode": "architecture = '___'",
        "solution": "architecture = 'stateful'",
        "hint": "It needs to remember, so it's stateful.",
        "rubric": "Identifies stateful."
      }
    ]
  },
  "Process Mapping": {
    ...
    "lessons": [
      {
        ...
        "theory": "## Don't Automate Broken Processes\nBefore building an AI automation, you must map the manual process step-by-step. If a process is chaotic and poorly defined, automating it will just execute a bad process faster.",
        "instructions": "## Task: Rule of Thumb\nAccording to Bill Gates: 'Automation applied to an efficient operation will magnify the efficiency. Automation applied to an inefficient operation will magnify the ___.'",
        "starterCode": "word = '___'",
        "solution": "word = 'inefficiency'",
        "hint": "inefficiency",
        "rubric": "Identifies inefficiency."
      },
      {
        "title": "Standard Operating Procedures",
        "theory": "## SOPs as Prompts\nA well-written Standard Operating Procedure (SOP) is essentially an algorithmic instruction manual for humans. In AI automation, human SOPs map perfectly into System Prompts for LLMs.",
        "instructions": "## Task: Translation\nWhat human document serves as the best starting template for an AI Agent's system prompt?",
        "starterCode": "answer = '___'",
        "solution": "answer = 'SOP'",
        "hint": "SOP (Standard Operating Procedure)",
    ...
    "lessons": [
      {
        ...
        "theory": "## The Centaur Approach\nTotal replacement is risky. Augmentation means the AI prepares 90% of the work (e.g., drafts a response), but a human must click 'Approve' or edit it before it goes out. This balances speed with quality control.",
        "instructions": "## Task: Strategy Definition\nWhat is the term for a workflow where an AI pauses execution and waits for human approval?",
        "starterCode": "term = 'Human-in-the-___'",
        "solution": "term = 'Human-in-the-Loop'",
        "hint": "Loop",
        "rubric": "Identifies Human-in-the-Loop."
      },
      {
        "title": "Confidence Thresholds",
        "theory": "## Conditional Routing\nYou can design an automation to check the AI's confidence score. If confidence > 95%, send automatically. If < 95%, route to a human.",
        "instructions": "## Task: Dynamic Routing\nWrite a simple check that routes to a human if confidence is below 0.95.",
        "starterCode": "confidence = 0.82\nif confidence ___ 0.95:\n    route_to = '___'",
        "solution": "confidence = 0.82\nif confidence < 0.95:\n    route_to = 'Human'",
        "hint": "Use < and 'Human'",
        "rubric": "Correctly sets < and routes to Human."
      }
    ]
  },
  "Cost Analysis of Automation": {
    ...
    "lessons": [
      {
        ...
        "theory": "## Calculating Costs\nLLM API pricing is based on Tokens (roughly 0.75 words). You must calculate the cost of both the Input Prompt (which includes the system instructions and context) and the Output Tokens.",
        "instructions": "## Task: Calculate Cost\nIf input tokens cost $10 per 1M and output tokens cost $30 per 1M, what is the cost of 100k input and 10k output tokens?",
        "starterCode": "input_cost = (100000 / 1000000) * 10\noutput_cost = (10000 / 1000000) * 30\ntotal = ___",
        "solution": "input_cost = (100000 / 1000000) * 10\noutput_cost = (10000 / 1000000) * 30\ntotal = 1.30",
        "hint": "1.00 + 0.30 = 1.30",
        "rubric": "Calculates 1.30."
      },
      {
        "title": "Return on Investment (ROI)",
        "theory": "## Business Value\nAn automation might cost $500/month in API calls, but if it saves 100 hours of human labor billed at $30/hour, the net savings is massive.",
        "instructions": "## Task: Calculate ROI\nA bot costs $200 but saves 50 hours of work at $20/hour. What is the net savings?",
        "starterCode": "human_cost = 50 * 20\nnet_savings = human_cost - ___\nanswer = ___",
        "solution": "human_cost = 50 * 20\nnet_savings = human_cost - 200\nanswer = 800",
        "hint": "1000 - 200 = 800",
        "rubric": "Calculates 800."
      }
    ]
  },
  "Failure States & Fallbacks": {
    ...
    "lessons": [
      {
        ...
        "theory": "## Expecting Errors\nAPIs fail. LLMs hallucinate. A robust automation uses 'Try/Catch' logic to gracefully degrade. If the AI API is down, the workflow should alert an admin rather than failing silently.",
        "instructions": "## Task: Error Path\nIf an API call returns a 500 error, what is the safest automation step to trigger next?",
        "starterCode": "# Options: Retry infinitely, Send alert to admin, Ignore and proceed\nnext_step = '___'",
        "solution": "# Options: Retry infinitely, Send alert to admin, Ignore and proceed\nnext_step = 'Send alert to admin'",
        "hint": "Send alert to admin",
        "rubric": "Identifies Send alert to admin."
      },
      {
        "title": "Exponential Backoff",
        "theory": "## Rate Limits\nIf you hit an API rate limit (e.g., 429 Too Many Requests), retrying immediately will just fail again. Exponential backoff means waiting 1s, then 2s, then 4s, etc., before retrying.",
        "instructions": "## Task: Backoff Multiplier\nIf your first wait is 1s, and you multiply the wait time by 2 after each failure, what is the wait time after the 3rd failure?",
        "starterCode": "wait_1 = 1\nwait_2 = 2\nwait_3 = ___",
        "solution": "wait_1 = 1\nwait_2 = 2\nwait_3 = 4",
        "hint": "Multiply 2 by 2 = 4",
        "rubric": "Calculates 4."
      }
    ]
  },
    ...
    "lessons": [
      {
        ...
        "theory": "## The GitHub of AI\nHugging Face is the central hub for open-source AI models. You can use the `transformers` library to easily download and run these models locally.",
        "instructions": "## Task: Load a Pipeline\nUse the pipeline function from the transformers library to load a sentiment-analysis model.",
        "starterCode": "from transformers import ___\n\nclassifier = ___('sentiment-analysis')",
        "solution": "from transformers import pipeline\n\nclassifier = pipeline('sentiment-analysis')",
        "hint": "Use pipeline",
        "rubric": "Correctly imports and uses the pipeline function."
      },
      {
        "title": "Ollama for Local Inference",
        "theory": "## Running Models Locally\nOllama allows you to run large language models (like Llama 3) locally on your own machine without relying on external APIs.",
        "instructions": "## Task: Ollama CLI\nWrite the terminal command used to download and run the `llama3` model using Ollama.",
        "starterCode": "___ run ___",
        "solution": "ollama run llama3",
        "hint": "ollama run llama3",
        "rubric": "Writes 'ollama run llama3'."
      }
    ]
  },
  "Semantic Routing": {
    ...
    "lessons": [
      {
        ...
        "theory": "## Steering the Conversation\nInstead of using a single giant prompt for everything, semantic routing uses embeddings to classify the user's intent (e.g., 'small talk' vs 'database query') and routes the request to a specialized prompt or tool.",
        "instructions": "## Task: Routing Logic\nIf a user asks 'What's the weather?', the router should direct it to the `weather_agent`. Assign the correct route.",
        "starterCode": "user_input = \"What's the weather?\"\nroute = classify_intent(user_input)\nif route == '___':\n    call_weather_agent()",
        "solution": "user_input = \"What's the weather?\"\nroute = classify_intent(user_input)\nif route == 'weather':\n    call_weather_agent()",
        "hint": "weather",
        "rubric": "Checks for the 'weather' route."
      },
      {
        "title": "Fast Embedding Search",
        "theory": "## Router Speed\nSemantic routers must be extremely fast. Instead of using a slow LLM to classify intent, they often embed the user's query and do a fast Cosine Similarity check against predefined intent examples.",
        "instructions": "## Task: Similarity Metric\nWhich mathematical metric is commonly used to compare two text embeddings in a semantic router?",
        "starterCode": "metric = '___ Similarity'",
        "solution": "metric = 'Cosine Similarity'",
    ...
    "lessons": [
      {
        ...
        "theory": "## Teaching the LLM\nTo allow an LLM to call a function, you must pass a JSON schema that describes the function's name, description, and required parameters.",
        "instructions": "## Task: JSON Schema\nComplete the schema definition for a `get_current_weather` function by specifying the parameter type as 'string'.",
        "starterCode": "tools = [{\n    \"type\": \"function\",\n    \"function\": {\n        \"name\": \"get_current_weather\",\n        \"parameters\": {\n            \"type\": \"object\",\n            \"properties\": {\n                \"location\": {\"type\": \"___\"}\n            }\n        }\n    }\n}]",
        "solution": "tools = [{\n    \"type\": \"function\",\n    \"function\": {\n        \"name\": \"get_current_weather\",\n        \"parameters\": {\n            \"type\": \"object\",\n            \"properties\": {\n                \"location\": {\"type\": \"string\"}\n            }\n        }\n    }\n}]",
        "hint": "The type is string",
        "rubric": "Sets the type to 'string'."
      },
      {
        "title": "Handling the Response",
        "theory": "## Executing the Function\nThe LLM does not execute the function itself; it returns a JSON object telling *your code* to execute the function with specific arguments.",
        "instructions": "## Task: Parse Arguments\nParse the JSON string returned by the LLM into a Python dictionary using the `json` module.",
        "starterCode": "import json\n\nllm_args_string = '{\"location\": \"Paris\"}'\nargs_dict = json.___(llm_args_string)",
        "solution": "import json\n\nllm_args_string = '{\"location\": \"Paris\"}'\nargs_dict = json.loads(llm_args_string)",
        "hint": "Use json.loads()",
        "rubric": "Correctly uses json.loads()."
      }
    ]
  },
  "Model Quantization": {
    ...
    "lessons": [
      {
        ...
        "theory": "## Shrinking Models\nQuantization reduces the memory footprint of an LLM by converting its weights from high-precision floats (e.g., 16-bit) to lower precision (e.g., 8-bit or 4-bit integer), allowing massive models to fit on consumer GPUs.",
        "instructions": "## Task: Bit Precision\nIf you apply INT4 quantization, how many bits are used to store each weight?",
        "starterCode": "bits = ___",
        "solution": "bits = 4",
        "hint": "INT4 means 4 bits",
        "rubric": "Identifies 4 bits."
      },
      {
        "title": "GGUF Format",
        "theory": "## Local Formats\nGGUF (GPT-Generated Unified Format) is a popular file format designed specifically for fast loading and saving of quantized LLMs, commonly used by `llama.cpp`.",
        "instructions": "## Task: Identify Format\nWhat is the acronym of the file format optimized for loading quantized models on CPUs and Apple Silicon?",
        "starterCode": "format = '___'",
        "solution": "format = 'GGUF'",
        "hint": "GGUF",
        "rubric": "Identifies GGUF."
      }
    ]
  },
  "Graph RAG": {
    ...
    "lessons": [
      {
        ...
        "theory": "## Entities and Relationships\nUnlike standard RAG (which chunks text into vector embeddings), Graph RAG extracts entities (Nodes) and their relationships (Edges) to form a structured Knowledge Graph.",
        "instructions": "## Task: Graph Components\nIn a Knowledge Graph representing 'Alice knows Bob', what is 'knows' considered?",
        "starterCode": "# Options: Node, Edge, Vector\ncomponent = '___'",
        "solution": "# Options: Node, Edge, Vector\ncomponent = 'Edge'",
        "hint": "It is a relationship, so it's an Edge.",
        "rubric": "Identifies Edge."
      },
      {
        "title": "Combining Vectors and Graphs",
        "theory": "## The Best of Both Worlds\nAdvanced systems combine Semantic Search (Vector DB) with Structural Search (Graph DB). The vector search finds relevant nodes, and the graph search retrieves the connected neighborhood to provide deep context to the LLM.",
        "instructions": "## Task: Multi-Hop Retrieval\nWhen answering complex questions like 'Who is the CEO of the company that acquired WhatsApp?', which RAG approach is generally better at tracing this 'multi-hop' connection?",
        "starterCode": "# Options: Standard RAG, Graph RAG\nbest_approach = '___'",
        "solution": "# Options: Standard RAG, Graph RAG\nbest_approach = 'Graph RAG'",
        "hint": "Graph RAG",
        "rubric": "Identifies Graph RAG."
      }
    ]
  },
  "Tokenization Basics": {
    ...
    "lessons": [
      {
        ...
        "theory": "## How LLMs Read\nLLMs don't read words; they read 'tokens'. A token is a chunk of text. In English, 1 token is roughly 4 characters or 0.75 words. A long word like 'unbelievable' might be split into multiple tokens.",
        "instructions": "## Task: Token Mapping\nIf an LLM uses Byte Pair Encoding (BPE), what is the most likely way it tokenizes the word 'hamburger'?",
        "starterCode": "# Options: [hamburger], [ham, burger], [h,a,m,b,u,r,g,e,r]\ntokens = '___'",
        "solution": "# Options: [hamburger], [ham, burger], [h,a,m,b,u,r,g,e,r]\ntokens = '[ham, burger]'",
        "hint": "It splits into common subwords: [ham, burger]",
        "rubric": "Identifies [ham, burger]."
      },
      {
        "title": "Context Windows",
        "theory": "## Short-Term Memory\nEvery LLM has a maximum context window limit (e.g., 128k tokens). This limit includes BOTH the input prompt you send and the output response it generates.",
        "instructions": "## Task: Context Limit Calculation\nIf a model has a 4096 token limit and your input prompt is 3000 tokens, what is the maximum number of tokens it can generate in its response?",
        "starterCode": "max_output = 4096 - ___",
        "solution": "max_output = 4096 - 3000",
        "hint": "Subtract 3000",
        "rubric": "Subtracts 3000."
      }
    ]
  },
  "Caching LLM Responses": {
    ...
    "lessons": [
      {
        ...
        "theory": "## Saving Money & Time\nIf thousands of users ask 'What is the capital of France?', you shouldn't call the OpenAI API every time. You can cache the exact string in Redis and return it instantly.",
        "instructions": "## Task: Cache Check\nWrite an if-statement that checks if the `user_prompt` exists in the `cache` dictionary.",
        "starterCode": "cache = {'Hello': 'Hi there!'}\nuser_prompt = 'Hello'\nif user_prompt ___ ___:\n    return cache[user_prompt]",
        "solution": "cache = {'Hello': 'Hi there!'}\nuser_prompt = 'Hello'\nif user_prompt in cache:\n    return cache[user_prompt]",
        "hint": "Use 'in cache'",
        "rubric": "Correctly checks if user_prompt in cache."
      },
      {
        "title": "Semantic Caching",
        "theory": "## Similar Intent\nExact match caching fails if someone asks 'What's the capital of France?' instead of 'What is the capital of France?'. Semantic caching solves this by comparing the *embedding* of the new query to cached embeddings. If they are 99% similar, it returns the cached response.",
        "instructions": "## Task: Similarity Metric\nWhat metric does a Semantic Cache use to compare the new query against cached queries?",
        "starterCode": "metric = '___ Similarity'",
        "solution": "metric = 'Cosine Similarity'",
        "hint": "Cosine Similarity",
        "rubric": "Identifies Cosine Similarity."
  ...
  "Structured Outputs": {
    ...
    "lessons": [
      {
        "title": "JSON Mode",
        "theory": "## Predictable Parsing\nIf you want an LLM to extract data from a resume, you need it returned in a strict JSON format so your code can parse it. Many APIs offer a 'JSON Mode' to guarantee this.",
        "instructions": "## Task: Enable JSON Mode\nIn the OpenAI API, how do you specify the response format to ensure valid JSON?",
        "starterCode": "response_format = { 'type': '___' }",
        "solution": "response_format = { 'type': 'json_object' }",
        "hint": "The type is json_object",
        "rubric": "Sets type to json_object."
      },
      {
        "title": "Constrained Generation",
        "theory": "## Guiding the Grammar\nAdvanced tools like `guidance` or `outlines` force the LLM to only output tokens that adhere to a specific Pydantic model or Regex pattern, completely eliminating parsing errors.",
        "instructions": "## Task: Regex Constraint\nIf you constrain the LLM to only output matching `^\\d{4}$`, what is it generating?",
        "starterCode": "# Options: A 4-letter word, A 4-digit number, A JSON object\noutput_type = '___'",
        "solution": "# Options: A 4-letter word, A 4-digit number, A JSON object\noutput_type = 'A 4-digit number'",
        "hint": "It generates A 4-digit number",
        "rubric": "Identifies A 4-digit number."
      }
    ]
  },
  "LoRA & QLoRA": {
    ...
    "lessons": [
      {
        ...
        "theory": "## Freezing the Base Model\nFine-tuning a 70B model normally requires massive GPU clusters. LoRA freezes the original weights and injects tiny, trainable 'rank decomposition matrices' into the layers, reducing trainable parameters by 99%.",
        "instructions": "## Task: LoRA Memory Benefit\nWhy does LoRA allow you to fine-tune large models on a single consumer GPU?",
        "starterCode": "# Options: It compresses the dataset, It drastically reduces trainable parameters\nreason = '___'",
        "solution": "# Options: It compresses the dataset, It drastically reduces trainable parameters\nreason = 'It drastically reduces trainable parameters'",
        "hint": "It drastically reduces trainable parameters",
        "rubric": "Identifies that it reduces trainable parameters."
      },
      {
        "title": "QLoRA",
        "theory": "## Quantized LoRA\nQLoRA takes it a step further: it loads the base model in 4-bit precision (Quantized), and trains the tiny LoRA adapters in 16-bit precision. This maximizes memory efficiency.",
        "instructions": "## Task: QLoRA Base Model\nIn QLoRA, what bit precision is the massive *base model* typically loaded into?",
        "starterCode": "precision = '___-bit'",
        "solution": "precision = '4-bit'",
        "hint": "4-bit",
        "rubric": "Identifies 4-bit."
      }
    ]
  },
  "Multi-Modal Models": {
    ...
    "lessons": [
      {
        ...
        "theory": "## Seeing the World\nVLMs (like GPT-4V or LLaVA) use an Image Encoder (like CLIP) to convert images into embeddings, which are then passed alongside text embeddings into the LLM.",
        "instructions": "## Task: The Image Encoder\nWhat architecture is most commonly used to encode images into embeddings for a Vision-Language Model?",
        "starterCode": "# Options: RNN, Vision Transformer (ViT), LSTM\nencoder = '___'",
        "solution": "# Options: RNN, Vision Transformer (ViT), LSTM\nencoder = 'Vision Transformer (ViT)'",
        "hint": "Vision Transformer (ViT)",
        "rubric": "Identifies Vision Transformer (ViT)."
      },
      {
        "title": "Joint Embedding Spaces",
        "theory": "## Cross-Modal Search\nModels like CLIP map text and images into the *exact same* embedding space. This means the embedding for the word 'Dog' and the embedding for a picture of a dog will have a high cosine similarity.",
        "instructions": "## Task: Search Application\nIf text and images are in the same embedding space, what powerful feature does this enable?",
        "starterCode": "# Options: Image Generation, Text-to-Image Search\nfeature = '___'",
        "solution": "# Options: Image Generation, Text-to-Image Search\nfeature = 'Text-to-Image Search'",
        "hint": "Text-to-Image Search",
        "rubric": "Identifies Text-to-Image Search."
      }
    ]
  },
  "HTTP & APIs": {
    ...
    "lessons": [
      {
        ...
        "theory": "## State vs Stateless\nSessions are 'stateful' (the server stores the session ID in a database). JWTs (JSON Web Tokens) are 'stateless' (the token itself contains all the user data). JWTs are great for microservices but are harder to revoke.",
        "instructions": "## Task: Token Format\nA JWT consists of three parts separated by dots. Which of these is NOT one of the parts?",
        "starterCode": "# Options: Header, Payload, Signature, DatabaseID\nnot_a_part = '___'",
        "solution": "# Options: Header, Payload, Signature, DatabaseID\nnot_a_part = 'DatabaseID'",
        "hint": "A JWT does not contain a DatabaseID",
        "rubric": "Identifies DatabaseID."
      },
      {
        "title": "OAuth 2.0 Flow",
        "theory": "## Delegated Access\nOAuth 2.0 allows a user to grant a third-party application access to their data without giving away their password (e.g., 'Log in with Google').",
        "instructions": "## Task: The Exchange\nIn the Authorization Code flow, the client receives an 'Authorization Code' first. What does it exchange this code for?",
        "starterCode": "# Options: An Access Token, A Password, A Session Cookie\nexchanges_for = '___'",
        "solution": "# Options: An Access Token, A Password, A Session Cookie\nexchanges_for = 'An Access Token'",
        "hint": "It exchanges it for An Access Token",
        "rubric": "Identifies An Access Token."
      },
      {
        "title": "Role-Based Access Control (RBAC)",
        "theory": "## Permissions\nAuthentication verifies *who* you are. Authorization verifies *what you can do*. RBAC assigns permissions to Roles (e.g., 'Admin', 'Editor') and then assigns users to those roles.",
        "instructions": "## Task: Authorization Middleware\nWrite the logic to block the request if the user's role is not 'admin'.",
        "starterCode": "function requireAdmin(req, res, next) {\n  if (req.user.role !== '___') {\n    return res.status(403).send('Forbidden');\n  }\n  next();\n}",
        "solution": "function requireAdmin(req, res, next) {\n  if (req.user.role !== 'admin') {\n    return res.status(403).send('Forbidden');\n  }\n  next();\n}",
        "hint": "Check if role is not equal to 'admin'",
        "rubric": "Checks against 'admin'."
      },
      {
        "title": "CSRF Protection",
        "theory": "## Cross-Site Request Forgery\nCSRF happens when a malicious site tricks a user's browser into making an unwanted request to your site. Since the browser automatically includes cookies, the request looks legit.",
        "instructions": "## Task: Anti-Forgery Token\nThe most common defense is requiring a unique, hidden token in every state-changing request (like POST). What is this token called?",
        "starterCode": "token_name = '___ Token'",
        "solution": "token_name = 'CSRF Token'",
        "hint": "It is called a CSRF Token",
        "rubric": "Identifies CSRF Token."
      },
      {
        "title": "XSS Prevention",
        "theory": "## Cross-Site Scripting\nXSS occurs when an attacker injects malicious JavaScript into your database, which is then rendered and executed by other users' browsers.",
        "instructions": "## Task: Defense Mechanism\nWhat is the primary defense against XSS when outputting user data in a template engine?",
        "starterCode": "# Options: Database indexing, HTML Escaping (Sanitization), CSRF Tokens\ndefense = '___'",
        "solution": "# Options: Database indexing, HTML Escaping (Sanitization), CSRF Tokens\ndefense = 'HTML Escaping (Sanitization)'",
        "hint": "You must sanitize or escape HTML",
        "rubric": "Identifies HTML Escaping (Sanitization)."
      },
      {
        "title": "Password Hashing",
        "theory": "## Never Store Plaintext\nYou must never store user passwords in plaintext. You should hash them using a slow algorithm like Bcrypt or Argon2, and always include a unique 'salt' to defeat rainbow table attacks.",
        "instructions": "## Task: Bcrypt Rounds\nIn bcrypt, you can increase the 'cost factor' (rounds) to make hashing slower, which defends against brute force. Set the salt rounds to 12.",
        "starterCode": "const bcrypt = require('bcrypt');\nconst saltRounds = ___;\nconst hash = await bcrypt.hash(myPlaintextPassword, saltRounds);",
        "solution": "const bcrypt = require('bcrypt');\nconst saltRounds = 12;\nconst hash = await bcrypt.hash(myPlaintextPassword, saltRounds);",
        "hint": "Set saltRounds to 12",
        "rubric": "Sets saltRounds to 12."
      },
      {
        "title": "Rate Limiting",
        "theory": "## Preventing Abuse\nRate limiting restricts how many requests a user or IP can make in a given timeframe. It prevents brute-force login attempts and DDoS attacks.",
        "instructions": "## Task: HTTP Status Code\nWhen a user exceeds their rate limit, what standard HTTP status code should your API return?",
        "starterCode": "# Options: 400, 403, 429\nstatus_code = ___",
        "solution": "# Options: 400, 403, 429\nstatus_code = 429",
        "hint": "429 means Too Many Requests",
        "rubric": "Identifies 429."
      },
      {
        "title": "Multi-Factor Authentication (MFA)",
        "theory": "## Layered Security\nMFA requires users to provide two or more verification factors to gain access. A common implementation uses TOTP (Time-based One-Time Password) generated by an authenticator app.",
        "instructions": "## Task: TOTP Secret\nTo set up TOTP, the backend generates a secret key and usually shares it with the user's authenticator app via what method?",
        "starterCode": "# Options: A phone call, A QR Code, A text message\nsetup_method = '___'",
        "solution": "# Options: A phone call, A QR Code, A text message\nsetup_method = 'A QR Code'",
        "hint": "It is shared via A QR Code",
        "rubric": "Identifies A QR Code."
      }
    ]
  },
  "System Observability & Monitoring Masterclass": {
    ...
    "lessons": [
      {
        ...
        "theory": "## Seeing Inside the Black Box\nObservability is defined by three primary pillars: Logs (record of events), Metrics (numerical measurements over time), and Traces (the end-to-end journey of a single request).",
        "instructions": "## Task: Identify the Pillar\nWhich pillar of observability is best used to track the exact millisecond breakdown of a request as it hops between 5 different microservices?",
        "starterCode": "# Options: Logs, Metrics, Traces\npillar = '___'",
        "solution": "# Options: Logs, Metrics, Traces\npillar = 'Traces'",
        "hint": "Traces track journeys across services",
        "rubric": "Identifies Traces."
      },
      {
        "title": "Structured Logging",
        "theory": "## Stop Using Print()\nUnstructured text logs like 'User 123 logged in' are hard to parse. Structured logging outputs logs in JSON format, making it trivial for log aggregators to query and filter.",
        "instructions": "## Task: JSON Log\nWhat format does structured logging typically output to ensure machines can easily parse the logs?",
        "starterCode": "format = '___'",
        "solution": "format = 'JSON'",
        "hint": "JSON",
        "rubric": "Identifies JSON."
      },
      {
        "title": "Distributed Tracing",
        "theory": "## OpenTelemetry\nWhen a request hits your API gateway, a 'Trace ID' is generated and passed in the headers to every downstream microservice. This allows tracing tools (like Jaeger) to stitch the entire request together.",
        "instructions": "## Task: Context Propagation\nWhat is the name of the unique identifier that must be passed along to every service to link the logs/spans together?",
        "starterCode": "identifier = '___ ID'",
        "solution": "identifier = 'Trace ID'",
        "hint": "Trace ID",
        "rubric": "Identifies Trace ID."
      },
      {
        "title": "Application Metrics",
        "theory": "## Measuring Health\nMetrics are aggregated numerical data. A 'Counter' only goes up (e.g., total requests). A 'Gauge' can go up and down (e.g., active websocket connections). A 'Histogram' measures distributions (e.g., response times).",
        "instructions": "## Task: Metric Types\nIf you want to track the current amount of RAM your server is using, which metric type should you use?",
        "starterCode": "# Options: Counter, Gauge, Histogram\nmetric_type = '___'",
        "solution": "# Options: Counter, Gauge, Histogram\nmetric_type = 'Gauge'",
        "hint": "It can go up and down, so it's a Gauge.",
        "rubric": "Identifies Gauge."
      },
      {
        "title": "Health Checks",
        "theory": "## Liveness vs Readiness\nIn orchestrated environments (like Kubernetes), a 'Liveness' probe checks if the app has crashed (restarts it if failing). A 'Readiness' probe checks if the app is ready to receive traffic (stops sending traffic if failing, e.g., during DB disconnects).",
        "instructions": "## Task: Probe Purpose\nIf your API is running but the Database connection drops, which probe should fail so that the load balancer temporarily stops sending traffic?",
        "starterCode": "# Options: Liveness Probe, Readiness Probe\nprobe = '___'",
        "solution": "# Options: Liveness Probe, Readiness Probe\nprobe = 'Readiness Probe'",
        "hint": "Readiness Probe",
        "rubric": "Identifies Readiness Probe."
      },
      {
        "title": "Alerting Strategies",
        "theory": "## Symptom-Based Alerting\nDon't alert when CPU hits 80%. Alert when the 99th percentile response time exceeds 2 seconds, or when the error rate exceeds 1%. Alert on *symptoms* that affect the user, not *causes*.",
        "instructions": "## Task: Alert Focus\nAccording to best practices, should you page an engineer at 3 AM because 'Database CPU is at 95%' or because 'User checkout is failing'?",
        "starterCode": "# Options: Database CPU, User checkout is failing\npage_for = '___'",
        "solution": "# Options: Database CPU, User checkout is failing\npage_for = 'User checkout is failing'",
        "hint": "User checkout is failing",
        "rubric": "Identifies User checkout is failing."
      },
      {
        "title": "Log Aggregation",
        "theory": "## The ELK Stack\nIn distributed systems, logs are scattered across many servers. Log aggregators (like Elasticsearch, Logstash, Kibana or Datadog) ingest all logs into a central searchable database.",
        "instructions": "## Task: ELK Acronym\nIn the popular open-source ELK stack, 'L' stands for Logstash, and 'K' stands for Kibana. What does the 'E' stand for?",
        "starterCode": "e_stands_for = '___'",
        "solution": "e_stands_for = 'Elasticsearch'",
        "hint": "Elasticsearch",
        "rubric": "Identifies Elasticsearch."
      },
      {
        "title": "Handling Outages",
        "theory": "## Incident Response\nWhen a massive outage occurs, the primary goal is *mitigation* (stopping the bleeding), not *resolution* (fixing the root cause). After the incident, a blameless post-mortem is written to prevent it from happening again.",
        "instructions": "## Task: The Review\nWhat is the industry term for the document written after an outage to analyze what went wrong without pointing fingers?",
        "starterCode": "term = 'Blameless ___-___'",
        "solution": "term = 'Blameless Post-Mortem'",
        "hint": "Post-Mortem",
        "rubric": "Identifies Post-Mortem or Postmortem."
      }
    ]
  },
  "AWS Basics": {
    ...
    "lessons": [
      {
        ...
        "theory": "## Adding Noise\nDiffusion models work by learning how to reverse noise. The 'Forward Process' (or diffusion process) takes a clean image and incrementally adds Gaussian noise to it over many time steps until it becomes pure static.",
        "instructions": "## Task: The End State\nIf you run the forward diffusion process for an infinite number of steps, what does the image eventually become?",
        "starterCode": "# Options: A black image, A white image, Pure Gaussian noise\nend_state = '___'",
        "solution": "# Options: A black image, A white image, Pure Gaussian noise\nend_state = 'Pure Gaussian noise'",
        "hint": "Pure Gaussian noise",
        "rubric": "Identifies Pure Gaussian noise."
      },
      {
        "title": "The Reverse Process",
        "theory": "## Denoising with UNet\nThe 'Reverse Process' is the actual AI generation. A neural network (typically a U-Net) is trained to predict the noise that was added at each step, so it can subtract that noise to recover a clean image.",
        "instructions": "## Task: Network Architecture\nWhat neural network architecture is traditionally used in Diffusion models to predict and remove noise?",
        "starterCode": "# Options: ResNet50, U-Net, LSTM\narchitecture = '___'",
        "solution": "# Options: ResNet50, U-Net, LSTM\narchitecture = 'U-Net'",
        "hint": "U-Net",
        "rubric": "Identifies U-Net."
      },
      {
        "title": "Latent Diffusion",
        "theory": "## Saving VRAM\nOperating on 1024x1024 images pixel-by-pixel is extremely slow and requires massive GPUs. 'Latent Diffusion' (used by Stable Diffusion) solves this by compressing the image into a smaller 'Latent Space' using a VAE, doing the diffusion there, and then decoding it.",
        "instructions": "## Task: The Compressor\nWhat component compresses the pixel image into the smaller latent space before the diffusion process begins?",
        "starterCode": "# Options: The CLIP text encoder, The VAE Encoder, The U-Net\ncompressor = 'The ___'",
        "solution": "# Options: The CLIP text encoder, The VAE Encoder, The U-Net\ncompressor = 'The VAE Encoder'",
        "hint": "VAE Encoder",
        "rubric": "Identifies VAE Encoder."
      },
      {
        "title": "Text Encoders",
        "theory": "## Cross-Attention\nTo generate images from text (like 'A cat on a moon'), the text must be converted into numerical embeddings. A text encoder like OpenAI's CLIP is used to create embeddings that the U-Net uses as a guide during denoising.",
        "instructions": "## Task: Guiding the Image\nIn Stable Diffusion, what specific mechanism inside the U-Net injects the text embeddings into the image generation process?",
        "starterCode": "# Options: Max Pooling, Cross-Attention, Softmax\nmechanism = '___'",
        "solution": "# Options: Max Pooling, Cross-Attention, Softmax\nmechanism = 'Cross-Attention'",
        "hint": "Cross-Attention",
        "rubric": "Identifies Cross-Attention."
      },
      {
        "title": "Classifier-Free Guidance",
        "theory": "## Following the Prompt\nClassifier-Free Guidance (CFG) controls how strictly the model should follow your text prompt. The model generates two images simultaneously (one with the prompt, one unconditionally) and extrapolates the difference.",
        "instructions": "## Task: CFG Scale Effect\nIf you set the CFG Scale extremely high (e.g., 20), what is the most likely outcome?",
        "starterCode": "# Options: The image ignores the prompt entirely, The image strictly follows the prompt but may look fried/oversaturated\noutcome = '___'",
        "solution": "# Options: The image ignores the prompt entirely, The image strictly follows the prompt but may look fried/oversaturated\noutcome = 'The image strictly follows the prompt but may look fried/oversaturated'",
        "hint": "It strictly follows the prompt but may look fried",
        "rubric": "Identifies strict following but fried/oversaturated."
      },
      {
        "title": "Schedulers and Samplers",
        "theory": "## Taking Steps\nThe scheduler (or sampler) determines how the noise is removed over time. Euler is very fast (good images in 20 steps), while DDIM might take longer but offers different stylistic convergences.",
        "instructions": "## Task: Sampler Speed\nWhy are modern samplers like Euler Ancestral or DPM++ preferred over the original DDPM sampler?",
        "starterCode": "# Options: They generate higher resolution images, They require far fewer steps to converge to a good image\nreason = '___'",
        "solution": "# Options: They generate higher resolution images, They require far fewer steps to converge to a good image\nreason = 'They require far fewer steps to converge to a good image'",
        "hint": "They require far fewer steps",
        "rubric": "Identifies fewer steps."
      },
      {
        "title": "Inpainting",
        "theory": "## Modifying Regions\nInpainting allows you to mask out a specific part of an image (like a person's hat) and run the diffusion process *only* on that masked area, seamlessly blending the new generation with the original image.",
        "instructions": "## Task: Required Inputs\nTo perform Inpainting, you must provide the model with a text prompt, the original image, and what else?",
        "starterCode": "# Options: A 3D model, A binary mask image, An audio file\nthird_input = '___'",
        "solution": "# Options: A 3D model, A binary mask image, An audio file\nthird_input = 'A binary mask image'",
        "hint": "A binary mask image",
        "rubric": "Identifies binary mask image."
      },
      {
        "title": "ControlNet",
        "theory": "## Spatial Guidance\nText prompts are bad at dictating exact spatial layouts. ControlNet solves this by locking the main diffusion model and feeding it a spatial guide image (like a Canny edge map or OpenPose skeleton).",
        "instructions": "## Task: Pose Guidance\nIf you want the generated person to stand in a very specific martial arts stance, which ControlNet preprocessor should you use?",
        "starterCode": "# Options: Depth Map, Canny Edge, OpenPose\npreprocessor = '___'",
        "solution": "# Options: Depth Map, Canny Edge, OpenPose\npreprocessor = 'OpenPose'",
        "hint": "OpenPose extracts human skeletons",
        "rubric": "Identifies OpenPose."
      }
    ]
  },
  "C Syntax": {
    ...
    "lessons": [
      {
        ...
        "theory": "## True Concurrency\nUnlike async/await in high-level languages, POSIX threads (pthreads) in C provide true OS-level concurrency. You spawn a new thread by calling `pthread_create()` and passing it a function pointer.",
        "instructions": "## Task: The Header\nTo use the pthreads library in C, you must include a specific header file.",
        "starterCode": "#include <___>",
        "solution": "#include <pthread.h>",
        "hint": "Include pthread.h",
        "rubric": "Uses <pthread.h>."
      },
      {
        "title": "Thread Joining and Detaching",
        "theory": "## Waiting for Completion\nIf your `main()` function returns before your threads finish, the OS will kill all running threads. You must use `pthread_join()` to block the main thread until the specified thread completes. Alternatively, `pthread_detach()` lets a thread run independently.",
        "instructions": "## Task: Join the Thread\nBlock the current thread until `thread1` finishes execution.",
        "starterCode": "___(thread1, NULL);",
        "solution": "pthread_join(thread1, NULL);",
        "hint": "Use pthread_join",
        "rubric": "Uses pthread_join."
      },
      {
        "title": "Race Conditions",
        "theory": "## Shared Memory Chaos\nA race condition occurs when multiple threads read and write to the same shared memory location simultaneously. The final value depends on the unpredictable timing (the 'race') of the OS scheduler.",
        "instructions": "## Task: The Solution\nTo prevent a race condition, you must protect the shared resource. What is the fundamental mechanism used to ensure only one thread can access the resource at a time?",
        "starterCode": "# Options: A switch statement, Mutual Exclusion (Mutex), A detached thread\nmechanism = '___'",
        "solution": "# Options: A switch statement, Mutual Exclusion (Mutex), A detached thread\nmechanism = 'Mutual Exclusion (Mutex)'",
        "hint": "Mutual Exclusion (Mutex)",
        "rubric": "Identifies Mutual Exclusion (Mutex)."
      },
      {
        "title": "Mutexes",
        "theory": "## The Lock\nA `pthread_mutex_t` is a lock. Before modifying a shared variable, a thread calls `pthread_mutex_lock()`. If another thread already holds the lock, the calling thread goes to sleep until it is unlocked with `pthread_mutex_unlock()`.",
        "instructions": "## Task: Release the Lock\nAfter updating the global counter, you must release the lock so other threads can proceed.",
        "starterCode": "pthread_mutex_lock(&lock);\ncounter++;\n___(&lock);",
        "solution": "pthread_mutex_lock(&lock);\ncounter++;\npthread_mutex_unlock(&lock);",
        "hint": "Use pthread_mutex_unlock",
        "rubric": "Uses pthread_mutex_unlock."
      },
      {
        "title": "Condition Variables",
        "theory": "## Waiting for State\nSometimes a thread needs to wait for a specific condition to become true (e.g., 'Wait until the queue is not empty'). Instead of spinning in a while loop (which wastes CPU), you use a `pthread_cond_t` to safely put the thread to sleep until another thread signals it.",
        "instructions": "## Task: The Signal\nWhat function is called by a producing thread to wake up at least one thread that is waiting on a condition variable?",
        "starterCode": "pthread_cond____(&cond);",
        "solution": "pthread_cond_signal(&cond);",
        "hint": "Use signal",
        "rubric": "Uses pthread_cond_signal."
      },
      {
        "title": "Semaphores",
        "theory": "## Counting Resources\nA Mutex allows exactly ONE thread in. A Semaphore (`sem_t`) maintains a counter and allows N threads in. It's often used to track the number of available slots in a bounded buffer.",
        "instructions": "## Task: Decrement the Semaphore\nIn POSIX semaphores, what function is used to decrement the counter and wait if it is zero? (Often called the P operation).",
        "starterCode": "sem____(&my_semaphore);",
        "solution": "sem_wait(&my_semaphore);",
        "hint": "Use wait",
        "rubric": "Uses sem_wait."
      },
      {
        "title": "Deadlocks",
        "theory": "## The Deadly Embrace\nA deadlock occurs when Thread A holds Lock 1 and is waiting for Lock 2, while Thread B holds Lock 2 and is waiting for Lock 1. Both sleep forever.",
        "instructions": "## Task: Prevention\nWhat is the most common and robust strategy to prevent locking deadlocks in a complex system?",
        "starterCode": "# Options: Never use locks, Always acquire locks in the exact same strict order, Run on a single core\nstrategy = '___'",
        "solution": "# Options: Never use locks, Always acquire locks in the exact same strict order, Run on a single core\nstrategy = 'Always acquire locks in the exact same strict order'",
        "hint": "Always acquire locks in the exact same strict order",
        "rubric": "Identifies strict locking order."
      },
      {
        "title": "Atomic Operations",
        "theory": "## C11 Atomics\nSince C11, the `<stdatomic.h>` header provides atomic types (like `atomic_int`). These use hardware-level instructions to guarantee that simple operations (like `counter++`) happen atomically, eliminating the need for slow mutexes for simple counters.",
        "instructions": "## Task: The Header\nInclude the C11 header required to declare an `atomic_int`.",
        "starterCode": "#include <___>",
        "solution": "#include <stdatomic.h>",
        "hint": "Include stdatomic.h",
        "rubric": "Uses <stdatomic.h>."
      }
    ]
  },
  "ETL Pipelines": {
  ...
  "Data Quality & Validation": {
    ...
    "lessons": [
      {
        "title": "Missing Data",
        "theory": "## The Bane of Data Science\nReal-world data is messy. You must check for missing values (NaN/Null) before building pipelines.",
        "instructions": "## Task: Detect Nulls\nUse pandas to count the number of missing values in each column of a DataFrame.",
        "starterCode": "import pandas as pd\n\ndf = pd.DataFrame({'A': [1, None], 'B': [3, 4]})\nmissing_counts = df.___().sum()",
        "solution": "import pandas as pd\n\ndf = pd.DataFrame({'A': [1, None], 'B': [3, 4]})\nmissing_counts = df.isnull().sum()",
        "hint": "Use isnull() or isna()",
        "rubric": "Correctly uses isnull() or isna()."
      },
      {
        "title": "Great Expectations",
        "theory": "## Assertions for Data\nGreat Expectations is a Python library that allows you to define strict rules (expectations) for your data, much like unit tests for code.",
        "instructions": "## Task: Define Expectation\nWrite the expectation that checks if the 'age' column values are between 18 and 99.",
        "starterCode": "import great_expectations as ge\n\ndf = ge.dataset.PandasDataset({'age': [25, 40, 17]})\nresult = df.expect_column_values_to_be_between('___', min_value=___, max_value=___)",
        "solution": "import great_expectations as ge\n\ndf = ge.dataset.PandasDataset({'age': [25, 40, 17]})\nresult = df.expect_column_values_to_be_between('age', min_value=18, max_value=99)",
        "hint": "Column is age, min 18, max 99",
        "rubric": "Correctly sets the column name and boundaries."
      }
    ]
  },
  "dbt (Data Build Tool)": {
    ...
    "lessons": [
      {
        "title": "Transformations in SQL",
        "theory": "## The 'T' in ELT\ndbt allows data analysts and engineers to transform data in their warehouse simply by writing SELECT statements. dbt handles the boilerplate DDL.",
        "instructions": "## Task: dbt Model\nWrite a simple dbt model that selects all active users.",
        "starterCode": "-- models/active_users.sql\nSELECT * \nFROM {{ ref('___') }} \nWHERE status = '___'",
        "solution": "-- models/active_users.sql\nSELECT * \nFROM {{ ref('users') }} \nWHERE status = 'active'",
        "hint": "Ref 'users' and status 'active'",
        "rubric": "References the users table and active status."
      },
      {
        "title": "Testing Models",
        "theory": "## Data Integrity\ndbt comes with built-in tests for `unique`, `not_null`, `accepted_values`, and `relationships`.",
        "instructions": "## Task: YAML Config\nConfigure a dbt YAML file to enforce that the `id` column in `users` is unique and not null.",
        "starterCode": "version: 2\nmodels:\n  - name: users\n    columns:\n      - name: id\n        tests:\n          - ___\n          - ___",
        "solution": "version: 2\nmodels:\n  - name: users\n    columns:\n      - name: id\n        tests:\n          - unique\n          - not_null",
        "hint": "Use unique and not_null",
        "rubric": "Correctly defines unique and not_null tests."
      }
    ]
  },
  "Data Lakes & Warehouses": {
    ...
    "lessons": [
      {
        "title": "Schema on Read vs Write",
        "theory": "## Fundamental Differences\nA Data Warehouse expects structured data and enforces a schema before writing (Schema on Write). A Data Lake accepts unstructured data, and structure is applied when querying (Schema on Read).",
        "instructions": "## Task: Identify Architecture\nWhich system is best suited for storing petabytes of raw, unstructured log files and images?",
        "starterCode": "best_system = 'Data ___'",
        "solution": "best_system = 'Data Lake'",
        "hint": "Data Lake",
        "rubric": "Identifies Data Lake."
      },
      {
        "title": "Columnar Storage",
        "theory": "## Parquet & ORC\nAnalytical databases and data lakes heavily use columnar formats like Parquet, which store data by columns rather than rows. This drastically speeds up aggregations.",
        "instructions": "## Task: Save as Parquet\nUse pandas to save a DataFrame to a Parquet file.",
        "starterCode": "import pandas as pd\n\ndf = pd.DataFrame({'sales': [100, 200, 300]})\ndf.to____('sales.parquet')",
        "solution": "import pandas as pd\n\ndf = pd.DataFrame({'sales': [100, 200, 300]})\ndf.to_parquet('sales.parquet')",
        "hint": "Use to_parquet",
        "rubric": "Correctly uses to_parquet."
      }
    ]
  },
  "Model Monitoring & Drift": {
    ...
    "lessons": [
      {
        "title": "Data Drift",
        "theory": "## Changing Worlds\nData Drift occurs when the statistical properties of the input features change over time in production, causing your model's accuracy to degrade.",
        "instructions": "## Task: Identify Drift Type\nIf a model trained to predict housing prices in 2019 is now making predictions in 2024 (where inflation has changed the baseline), what kind of drift is this?",
        "starterCode": "# Options: Concept Drift, Data Drift\ndrift_type = '___'",
        "solution": "# Options: Concept Drift, Data Drift\ndrift_type = 'Data Drift'",
        "hint": "Data Drift",
        "rubric": "Identifies Data Drift."
      },
      {
        "title": "Evidently AI",
        "theory": "## Automated Detection\nLibraries like Evidently calculate statistical distance metrics (like Wasserstein distance) between a reference dataset and current production data.",
        "instructions": "## Task: Report Generation\nGenerate a data drift report using Evidently.",
        "starterCode": "from evidently.report import Report\nfrom evidently.metric_preset import DataDriftPreset\n\nreport = Report(metrics=[___()])\nreport.run(reference_data=ref_df, current_data=curr_df)",
        "solution": "from evidently.report import Report\nfrom evidently.metric_preset import DataDriftPreset\n\nreport = Report(metrics=[DataDriftPreset()])\nreport.run(reference_data=ref_df, current_data=curr_df)",
        "hint": "Use DataDriftPreset",
        "rubric": "Instantiates DataDriftPreset."
      }
    ]
  },
  "Kubeflow Pipelines": {
    ...
    "lessons": [
      {
        ...
        "theory": "## Kubernetes Native\nKubeflow Pipelines (KFP) is a platform for building and deploying scalable machine learning workflows on Kubernetes.",
        "instructions": "## Task: KFP Decorator\nUse the KFP SDK decorator to define a python function as a pipeline component.",
        "starterCode": "from kfp import dsl\n\n@dsl.___\ndef train_model(data: str) -> str:\n    return 'model_path'",
        "solution": "from kfp import dsl\n\n@dsl.component\ndef train_model(data: str) -> str:\n    return 'model_path'",
        "hint": "Use @dsl.component",
        "rubric": "Correctly uses the component decorator."
      },
      {
        "title": "Pipeline Definition",
        "theory": "## Stitching Components\nA Pipeline connects the outputs of one component to the inputs of another, creating a Directed Acyclic Graph (DAG).",
        "instructions": "## Task: Pass Data\nPass the output of the `preprocess` component into the `train` component.",
        "starterCode": "@dsl.pipeline(name='my-pipeline')\ndef my_pipeline():\n    data_task = preprocess()\n    train_task = train(data=data_task.___)",
        "solution": "@dsl.pipeline(name='my-pipeline')\ndef my_pipeline():\n    data_task = preprocess()\n    train_task = train(data=data_task.output)",
        "hint": "Use .output",
        "rubric": "Accesses the output property."
      }
    ]
  },
  "Distributed Computing & Spark Masterclass": {
    ...
    "lessons": [
      {
        ...
        "theory": "## In-Memory Speed\nHadoop MapReduce writes intermediate results to disk, which is very slow. Apache Spark revolutionized distributed computing by keeping intermediate data in-memory (RAM), making it up to 100x faster for iterative workloads like Machine Learning.",
        "instructions": "## Task: The Secret Sauce\nWhat is the primary architectural difference that makes Apache Spark so much faster than Hadoop MapReduce?",
        "starterCode": "# Options: Disk-based storage, In-memory computation, Using Python instead of Java\nadvantage = '___'",
        "solution": "# Options: Disk-based storage, In-memory computation, Using Python instead of Java\nadvantage = 'In-memory computation'",
        "hint": "In-memory computation",
        "rubric": "Identifies In-memory computation."
      },
      {
        "title": "Resilient Distributed Datasets (RDDs)",
        "theory": "## The Core Abstraction\nAn RDD is Spark's fundamental data structure. They are distributed across the cluster, immutable (cannot be changed once created), and fault-tolerant because they remember their 'lineage' (the steps used to create them).",
        "instructions": "## Task: Fault Tolerance\nIf a node in a Spark cluster crashes and loses an RDD partition, how does Spark recover the lost data?",
        "starterCode": "# Options: Recomputes it using the lineage graph, Asks the user to restart, Reads it from a backup database\nrecovery_method = '___'",
        "solution": "# Options: Recomputes it using the lineage graph, Asks the user to restart, Reads it from a backup database\nrecovery_method = 'Recomputes it using the lineage graph'",
        "hint": "Recomputes it using the lineage graph",
        "rubric": "Identifies Recomputes it using the lineage graph."
      },
      {
        "title": "Spark DataFrames and Catalyst",
        "theory": "## The Optimizer\nWhile RDDs give you low-level control, Spark DataFrames provide a higher-level API. Behind the scenes, the Catalyst Optimizer analyzes your DataFrame code and generates an optimized physical execution plan.",
        "instructions": "## Task: The Engine\nWhat is the name of the engine that optimizes DataFrame and Spark SQL queries before execution?",
        "starterCode": "engine = 'The ___ Optimizer'",
        "solution": "engine = 'The Catalyst Optimizer'",
        "hint": "Catalyst Optimizer",
        "rubric": "Identifies Catalyst."
      },
      {
        "title": "Lazy Evaluation",
        "theory": "## Waiting to Execute\nIn Spark, 'Transformations' (like `map` or `filter`) do not execute immediately; they just build the lineage graph. Spark only actually runs the computation when an 'Action' (like `count` or `collect`) is called. This is called Lazy Evaluation.",
        "instructions": "## Task: Action vs Transformation\nWhich of the following Spark operations will actually trigger computation across the cluster?",
        "starterCode": "# Options: .filter(), .select(), .count()\ntrigger = '___'",
        "solution": "# Options: .filter(), .select(), .count()\ntrigger = '.count()'",
        "hint": ".count() is an action",
        "rubric": "Identifies .count()."
      },
      {
        "title": "Partitioning and Shuffling",
        "theory": "## Network Bottlenecks\nSpark splits data into 'partitions' across different nodes. Operations like `groupByKey` or `join` require data to be moved between different nodes. This massive movement of data across the network is called a 'Shuffle' and is the biggest performance bottleneck.",
        "instructions": "## Task: The Bottleneck\nWhat is the term for the expensive operation where Spark redistributes data across the cluster's network?",
        "starterCode": "term = '___'",
        "solution": "term = 'Shuffle'",
        "hint": "Shuffle",
        "rubric": "Identifies Shuffle."
      },
      {
        "title": "Broadcast Variables",
        "theory": "## Distributing Lookups\nIf you have a small lookup table (like a list of zip codes) that every node needs for a Join, a standard join will cause a massive Shuffle. Instead, you can 'Broadcast' the small table, sending a read-only copy to every node's memory.",
        "instructions": "## Task: Avoid the Shuffle\nTo optimize a join between a 1TB table and a 10MB lookup table, what type of variable should you use for the 10MB table?",
        "starterCode": "variable_type = '___ Variable'",
        "solution": "variable_type = 'Broadcast Variable'",
        "hint": "Broadcast Variable",
        "rubric": "Identifies Broadcast Variable."
      },
      {
        "title": "PySpark SQL",
        "theory": "## SQL on Clusters\nYou don't have to learn the DataFrame API to use Spark. You can register a DataFrame as a temporary view and run standard ANSI SQL queries against terabytes of data using `spark.sql()`.",
        "instructions": "## Task: Query Execution\nWrite the method call to execute a raw SQL query string against registered views in PySpark.",
        "starterCode": "df = ___.sql('SELECT * FROM users WHERE age > 21')",
        "solution": "df = spark.sql('SELECT * FROM users WHERE age > 21')",
        "hint": "Use spark",
        "rubric": "Uses spark.sql()."
      },
      {
        "title": "Structured Streaming",
        "theory": "## Real-Time Pipelines\nSpark isn't just for batch processing. Structured Streaming allows you to process real-time data (e.g., from Kafka) using the exact same DataFrame API you use for batch processing, treating the stream as an infinitely growing table.",
        "instructions": "## Task: The Abstraction\nIn Structured Streaming, a real-time data stream is conceptually treated as an unbounded what?",
        "starterCode": "# Options: Unbounded Array, Unbounded Table, Unbounded Dictionary\nconcept = '___'",
        "solution": "# Options: Unbounded Array, Unbounded Table, Unbounded Dictionary\nconcept = 'Unbounded Table'",
        "hint": "Unbounded Table",
        "rubric": "Identifies Unbounded Table."
      }
    ]
  },
  "Pandas Intro": {
    ...
    "lessons": [
      {
        ...
        "theory": "## Finding Patterns\nUnsupervised learning models do not have target labels (y). K-Means algorithm partitions data into K distinct clusters based on feature similarity.",
        "instructions": "## Task: Initialize K-Means\nInitialize a KMeans model from scikit-learn with 3 clusters.",
        "starterCode": "from sklearn.cluster import KMeans\n\nkmeans = KMeans(n_clusters=___, random_state=42)",
        "solution": "from sklearn.cluster import KMeans\n\nkmeans = KMeans(n_clusters=3, random_state=42)",
        "hint": "Use 3 for n_clusters",
        "rubric": "Sets n_clusters to 3."
      },
      {
        "title": "DBSCAN",
        "theory": "## Density-Based Spatial Clustering\nUnlike K-Means which forces every point into a cluster (often forming spherical shapes), DBSCAN groups together points that are closely packed together, and marks points in low-density regions as outliers (noise).",
        "instructions": "## Task: Outlier Label\nIn scikit-learn's DBSCAN, what label is assigned to noisy/outlier points that don't belong to any cluster?",
        "starterCode": "outlier_label = ___",
        "solution": "outlier_label = -1",
        "hint": "It is -1",
        "rubric": "Identifies -1 as the outlier label."
      }
    ]
  },
  "Numpy Basics": {
    ...
    "lessons": [
      {
        "title": "The NDArray",
        "theory": "## Fast Math\nNumpy is the core library for scientific computing in Python. It provides a high-performance multidimensional array object, which is much faster than standard Python lists.",
        "instructions": "## Task: Create an Array\nImport numpy and create a 1D array from the list `[1, 2, 3]`.",
        "starterCode": "import numpy as np\n\narr = np.___( [1, 2, 3] )",
        "solution": "import numpy as np\n\narr = np.array( [1, 2, 3] )",
        "hint": "Use np.array",
        "rubric": "Correctly uses np.array()."
      },
      {
        "title": "Vectorized Operations",
        "theory": "## Broadcasting\nYou can perform mathematical operations directly on Numpy arrays without writing loops. If you multiply an array by 2, every element is multiplied by 2 instantly.",
        "instructions": "## Task: Multiply Array\nMultiply every element in `arr` by 10.",
        "starterCode": "import numpy as np\n\narr = np.array([1, 2, 3])\nresult = arr ___ 10",
        "solution": "import numpy as np\n\narr = np.array([1, 2, 3])\nresult = arr * 10",
        "hint": "Just use the * operator",
        "rubric": "Correctly uses arr * 10."
      }
    ]
  },
  "Handling Imbalanced Data": {
    ...
    "lessons": [
      {
        "title": "Class Weights",
        "theory": "## Penalizing Mistakes\nWhen 99% of your data is 'Normal' and 1% is 'Fraud', a model can just predict 'Normal' every time and get 99% accuracy. You can fix this by telling the algorithm to penalize mistakes on the minority class more heavily.",
        "instructions": "## Task: Balanced Weights\nInitialize a LogisticRegression model and set it to automatically adjust weights inversely proportional to class frequencies.",
        "starterCode": "from sklearn.linear_model import LogisticRegression\n\nmodel = LogisticRegression(class_weight='___')",
        "solution": "from sklearn.linear_model import LogisticRegression\n\nmodel = LogisticRegression(class_weight='balanced')",
        "hint": "Use the word 'balanced'",
        "rubric": "Sets class_weight to 'balanced'."
      },
      {
        "title": "SMOTE Oversampling",
        "theory": "## Generating Synthetic Data\nSMOTE (Synthetic Minority Over-sampling Technique) fixes imbalance not by removing data, but by generating synthetic examples of the minority class so both classes have equal representation.",
        "instructions": "## Task: Import SMOTE\nImport the SMOTE class from the `imblearn` (Imbalanced-Learn) library.",
        "starterCode": "from imblearn.over_sampling import ___",
        "solution": "from imblearn.over_sampling import SMOTE",
        "hint": "Import SMOTE",
        "rubric": "Imports SMOTE."
      }
    ]
  },
  "Model Evaluation Metrics": {
    ...
    "lessons": [
      {
        ...
        "theory": "## Beyond Accuracy\nPrecision asks: 'Out of all the emails I flagged as spam, how many were actually spam?'. Recall asks: 'Out of all the real spam emails, how many did I successfully flag?'.",
        "instructions": "## Task: Calculate F1 Score\nThe F1 Score is the harmonic mean of Precision and Recall. Import the function to calculate it.",
        "starterCode": "from sklearn.metrics import ___",
        "solution": "from sklearn.metrics import f1_score",
        "hint": "Import f1_score",
        "rubric": "Imports f1_score."
      },
      {
        "title": "ROC and AUC",
        "theory": "## The ROC Curve\nThe Receiver Operating Characteristic (ROC) curve plots the True Positive Rate vs the False Positive Rate at different classification thresholds. The Area Under the Curve (AUC) summarizes this into a single number between 0 and 1.",
        "instructions": "## Task: Calculate AUC\nCalculate the ROC AUC score given the true labels `y_true` and predicted probabilities `y_probs`.",
        "starterCode": "from sklearn.metrics import roc_auc_score\n\nscore = roc_auc_score(___, ___)",
        "solution": "from sklearn.metrics import roc_auc_score\n\nscore = roc_auc_score(y_true, y_probs)",
        "hint": "Pass y_true, then y_probs",
        "rubric": "Correctly calls roc_auc_score(y_true, y_probs)."
      }
    ]
  },
  "Deep Learning for Tabular Data": {
    ...
    "lessons": [
      {
        ...
        "theory": "## Mapping Categories\nWhile One-Hot Encoding is standard for categorical variables, deep learning can use Entity Embeddings to map categories (like 'City' or 'Store ID') into dense vectors, capturing semantic relationships between them.",
        "instructions": "## Task: Embedding Layer\nIn PyTorch, which layer is used to create trainable embeddings for categorical variables?",
        "starterCode": "import torch.nn as nn\n\nlayer = nn.___ (num_categories, embedding_dim)",
        "solution": "import torch.nn as nn\n\nlayer = nn.Embedding (num_categories, embedding_dim)",
        "hint": "Use nn.Embedding",
        "rubric": "Identifies nn.Embedding."
      },
      {
        "title": "Autoencoders for Anomaly Detection",
        "theory": "## Reconstruction Error\nAn Autoencoder is a neural network trained to compress tabular data into a bottleneck and then reconstruct it. If it is fed anomalous data (like fraud), it will struggle to reconstruct it, resulting in a high 'Reconstruction Error'.",
        "instructions": "## Task: Bottleneck Size\nIn an autoencoder, the hidden layer representation (the bottleneck) must be ___ than the input dimension to force compression.",
        "starterCode": "# Options: larger, smaller, equal\nsize = '___'",
        "solution": "# Options: larger, smaller, equal\nsize = 'smaller'",
        "hint": "It must be smaller.",
        "rubric": "Identifies smaller."
      }
    ]
  },
  "Recommendation Systems": {
    ...
    "lessons": [
      {
        ...
        "theory": "## User-Item Matrices\nCollaborative filtering assumes that if User A and User B liked similar movies in the past, they will like similar movies in the future. It relies purely on interaction data.",
        "instructions": "## Task: Matrix Factorization\nWhat technique decomposes a sparse User-Item interaction matrix into two lower-dimensional matrices (User factors and Item factors)?",
        "starterCode": "# Options: PCA, Matrix Factorization, K-Means\ntechnique = '___'",
        "solution": "# Options: PCA, Matrix Factorization, K-Means\ntechnique = 'Matrix Factorization'",
        "hint": "Matrix Factorization",
        "rubric": "Identifies Matrix Factorization."
      },
      {
        "title": "Content-Based Filtering",
        "theory": "## Using Metadata\nContent-based filtering ignores other users and focuses on the items themselves. If you watched a Sci-Fi movie starring Keanu Reeves, it will recommend other Sci-Fi movies or movies with Keanu Reeves.",
        "instructions": "## Task: Similarity Metric\nIf you represent each movie as a TF-IDF vector of its plot keywords, which metric is commonly used to find similar movies?",
        "starterCode": "# Options: Euclidean Distance, Cosine Similarity\nmetric = '___'",
        "solution": "# Options: Euclidean Distance, Cosine Similarity\nmetric = 'Cosine Similarity'",
        "hint": "Cosine Similarity",
        "rubric": "Identifies Cosine Similarity."
      }
    ]
  ...
  "Pandas Data Manipulation Masterclass": {
    ...
    "lessons": [
      {
        "title": "loc vs iloc",
        "theory": "## Selecting Data\n`.loc` is label-based, meaning you specify the exact name of the row/column. `.iloc` is integer-based, meaning you specify the numeric index (0, 1, 2) regardless of the label.",
        "instructions": "## Task: Integer Selection\nSelect the first 5 rows and first 2 columns of a DataFrame `df` using integer-based indexing.",
        "starterCode": "subset = df.___[:5, :2]",
        "solution": "subset = df.iloc[:5, :2]",
        "hint": "Use iloc",
        "rubric": "Uses iloc."
      },
      {
        "title": "GroupBy & Aggregations",
        "theory": "## Split-Apply-Combine\nThe `.groupby()` method splits your data into groups, applies a function to each group independently (like `sum` or `mean`), and combines the results back into a new DataFrame.",
        "instructions": "## Task: Group and Mean\nGroup the DataFrame `df` by the 'Category' column and calculate the mean of the remaining columns.",
        "starterCode": "avg_by_cat = df.___('Category').___()",
        "solution": "avg_by_cat = df.groupby('Category').mean()",
        "hint": "Use groupby and mean",
        "rubric": "Uses groupby and mean()."
      },
      {
        "title": "Window Functions",
        "theory": "## Rolling Calculations\nWindow functions perform calculations across a set of rows related to the current row. For example, a 7-day rolling average smooths out daily volatility in a time series.",
        "instructions": "## Task: Rolling Mean\nCalculate a 7-period rolling average on the 'Sales' column.",
        "starterCode": "smoothed = df['Sales'].___(___).mean()",
        "solution": "smoothed = df['Sales'].rolling(7).mean()",
        "hint": "Use rolling(7)",
        "rubric": "Uses rolling(7)."
      },
      {
        "title": "Merging and Joining",
        "theory": "## Combining Datasets\n`pd.merge()` works similarly to SQL joins. A 'left' join keeps all rows from the left DataFrame, filling in missing matches from the right DataFrame with NaNs.",
        "instructions": "## Task: Perform a Left Join\nMerge `df1` and `df2` on the 'user_id' column using a left join.",
        "starterCode": "merged_df = pd.___(df1, df2, on='user_id', how='___')",
        "solution": "merged_df = pd.merge(df1, df2, on='user_id', how='left')",
        "hint": "Use merge and 'left'",
        "rubric": "Uses merge and left."
      },
      {
        "title": "Pivot Tables",
        "theory": "## Reshaping Data\n`pd.pivot_table()` creates spreadsheet-style pivot tables as a DataFrame. It is a powerful alternative to `groupby` when you need to specify rows, columns, and aggregate functions simultaneously.",
        "instructions": "## Task: Create a Pivot Table\nCreate a pivot table using 'City' as the index and computing the sum of 'Sales'.",
        "starterCode": "pivot = pd.___(df, values='Sales', index='City', aggfunc='___')",
        "solution": "pivot = pd.pivot_table(df, values='Sales', index='City', aggfunc='sum')",
        "hint": "Use pivot_table and 'sum'",
        "rubric": "Uses pivot_table and sum."
      },
      {
        "title": "MultiIndex",
        "theory": "## Hierarchical Indexing\nA MultiIndex allows you to have multiple levels of indexes on a single axis. This is useful for working with higher-dimensional data in a 2D DataFrame (like Year -> Month -> Sales).",
        "instructions": "## Task: Set Multiple Indexes\nSet both the 'Year' and 'Month' columns as the index of the DataFrame.",
        "starterCode": "df_multi = df.___( [ 'Year', 'Month' ] )",
        "solution": "df_multi = df.set_index( [ 'Year', 'Month' ] )",
        "hint": "Use set_index",
        "rubric": "Uses set_index."
      },
      {
        "title": "Vectorized String Methods",
        "theory": "## Fast Text Processing\nPandas allows you to apply string methods to an entire column at once using the `.str` accessor, bypassing slow Python loops.",
        "instructions": "## Task: String Contains\nFilter the DataFrame to keep only rows where the 'Email' column contains the string '@gmail.com'.",
        "starterCode": "gmail_users = df[ df['Email'].___.___('@gmail.com') ]",
        "solution": "gmail_users = df[ df['Email'].str.contains('@gmail.com') ]",
        "hint": "Use .str.contains",
        "rubric": "Uses .str.contains."
      },
      {
        "title": "Handling DateTimes",
        "theory": "## Time Series Foundations\nBy converting strings to true datetime objects, you unlock powerful methods for extracting years, months, days, or resampling data into different frequencies (like daily to monthly).",
        "instructions": "## Task: Convert to DateTime\nConvert the 'Date' column from raw strings into actual pandas datetime objects.",
        "starterCode": "df['Date'] = pd.___( df['Date'] )",
        "solution": "df['Date'] = pd.to_datetime( df['Date'] )",
        "hint": "Use to_datetime",
        "rubric": "Uses pd.to_datetime."
      }
    ]
  },
  "A/B Testing & Causal Inference Masterclass": {
    ...
    "lessons": [
      {
        ...
        "theory": "## The Scientific Method\nIn A/B testing, you start with a Null Hypothesis (H0) which states there is *no difference* between the control and variant. Your goal is to gather enough data to reject H0 in favor of the Alternative Hypothesis (H1).",
        "instructions": "## Task: The Default Assumption\nWhen testing a new button color to see if it increases click-through rates, what does the Null Hypothesis assume?",
        "starterCode": "# Options: The new color is better, The new color is worse, The color has no effect\nnull_hypothesis = '___'",
        "solution": "# Options: The new color is better, The new color is worse, The color has no effect\nnull_hypothesis = 'The color has no effect'",
        "hint": "It assumes no effect.",
        "rubric": "Identifies The color has no effect."
      },
      {
        "title": "p-values and Alpha",
        "theory": "## Measuring Surprise\nThe p-value is the probability of seeing your experimental results *assuming the Null Hypothesis is true*. If the p-value is lower than your significance level (Alpha, typically 0.05), you reject the Null Hypothesis.",
        "instructions": "## Task: Statistical Significance\nIf you run an A/B test with an Alpha of 0.05, and your calculated p-value is 0.02, what is the conclusion?",
        "starterCode": "# Options: Reject Null, Fail to reject Null\nconclusion = '___'",
        "solution": "# Options: Reject Null, Fail to reject Null\nconclusion = 'Reject Null'",
        "hint": "0.02 is less than 0.05, so Reject Null",
        "rubric": "Identifies Reject Null."
      },
      {
        "title": "Statistical Power",
        "theory": "## Avoiding False Negatives\nStatistical Power (1 - Beta) is the probability that your test will correctly detect a true effect when one exists. A common target for power is 80%. If your power is too low, you might abandon a winning feature.",
        "instructions": "## Task: Increasing Power\nWhat is the most direct way to increase the Statistical Power of an A/B test before running it?",
        "starterCode": "# Options: Decrease sample size, Increase sample size, Change the button color again\nway_to_increase = '___'",
        "solution": "# Options: Decrease sample size, Increase sample size, Change the button color again\nway_to_increase = 'Increase sample size'",
        "hint": "Increase sample size",
        "rubric": "Identifies Increase sample size."
      },
      {
        "title": "T-Tests",
        "theory": "## Comparing Means\nAn Independent Two-Sample T-Test is used to determine if there is a statistically significant difference between the *averages* of two independent groups (e.g., Average Revenue Per User in Group A vs Group B).",
        "instructions": "## Task: Run a T-Test\nImport the function from scipy to run an independent t-test.",
        "starterCode": "from scipy.stats import ___\n\nstat, pval = ___(group_a, group_b)",
        "solution": "from scipy.stats import ttest_ind\n\nstat, pval = ttest_ind(group_a, group_b)",
        "hint": "Use ttest_ind",
        "rubric": "Uses ttest_ind."
      },
      {
        "title": "Chi-Square Tests",
        "theory": "## Categorical Proportions\nIf your metric is a binary proportion (e.g., Conversion Rate: converted vs not converted), a T-Test isn't appropriate. Instead, you use a Chi-Square test of independence to compare the categorical distributions.",
        "instructions": "## Task: Test Selection\nWhich test should you use to compare the Conversion Rate (a percentage) between the Control and Variant groups?",
        "starterCode": "# Options: T-Test, Chi-Square Test, ANOVA\ntest_to_use = '___'",
        "solution": "# Options: T-Test, Chi-Square Test, ANOVA\ntest_to_use = 'Chi-Square Test'",
        "hint": "Chi-Square Test",
        "rubric": "Identifies Chi-Square Test."
      },
      {
        "title": "The Multiple Testing Problem",
        "theory": "## Peeking and Splitting\nIf you test 20 different button colors at a 0.05 significance level, you are almost guaranteed to get at least one false positive purely by chance. This is the Multiple Testing Problem.",
        "instructions": "## Task: The Correction\nWhat is the name of the conservative method that corrects for this by dividing your alpha by the number of tests (e.g., 0.05 / 20)?",
        "starterCode": "# Options: The Student Correction, The Bonferroni Correction, The Pearson Correction\ncorrection = 'The ___'",
        "solution": "# Options: The Student Correction, The Bonferroni Correction, The Pearson Correction\ncorrection = 'The Bonferroni Correction'",
        "hint": "Bonferroni Correction",
        "rubric": "Identifies Bonferroni Correction."
      },
      {
        "title": "Difference-in-Differences",
        "theory": "## Quasi-Experiments\nSometimes you can't run a randomized A/B test (e.g., a new law is passed in one state). Difference-in-Differences (DiD) compares the change over time in the treated group to the change over time in the control group to estimate the causal effect.",
        "instructions": "## Task: The Core Assumption\nDiD relies on the assumption that, in the absence of treatment, the difference between the two groups would have remained constant over time. What is this assumption called?",
        "starterCode": "# Options: Parallel Trends, Orthogonal Vectors, Linear Regression\nassumption = '___'",
        "solution": "# Options: Parallel Trends, Orthogonal Vectors, Linear Regression\nassumption = 'Parallel Trends'",
        "hint": "Parallel Trends",
        "rubric": "Identifies Parallel Trends."
      },
      {
        "title": "Propensity Score Matching",
        "theory": "## Observational Data\nWhen analyzing observational data where users *self-selected* into a feature, you have selection bias. Propensity Score Matching pairs a treated user with a non-treated user who has very similar characteristics to simulate a randomized control trial.",
        "instructions": "## Task: The Score\nWhat does the 'Propensity Score' actually represent mathematically?",
        "starterCode": "# Options: Probability of converting, Probability of receiving the treatment, The lifetime value of the user\nrepresents = '___'",
        "solution": "# Options: Probability of converting, Probability of receiving the treatment, The lifetime value of the user\nrepresents = 'Probability of receiving the treatment'",
        "hint": "Probability of receiving the treatment",
        "rubric": "Identifies Probability of receiving the treatment."
      }
    ]
  },
  "Big O Notation": {
    ...
    "lessons": [
      {
        ...
        "theory": "## The Widget Tree\nIn Flutter, everything you see on the screen is a Widget. A `StatelessWidget` never changes its appearance after it's built, while a `StatefulWidget` can dynamically update its UI based on internal state changes.",
        "instructions": "## Task: Base Class\nIf you want to create a UI component that maintains an internal counter, which class should it inherit from?",
        "starterCode": "# Options: StatelessWidget, StatefulWidget, InheritedWidget\nbase_class = '___'",
        "solution": "# Options: StatelessWidget, StatefulWidget, InheritedWidget\nbase_class = 'StatefulWidget'",
        "hint": "It needs state, so it's a StatefulWidget",
        "rubric": "Identifies StatefulWidget."
      },
      {
        "title": "Dart Language Basics",
        "theory": "## Sound Null Safety\nFlutter uses the Dart programming language. Modern Dart features 'Sound Null Safety', meaning variables cannot contain `null` by default unless you explicitly allow it using a question mark `?`.",
        "instructions": "## Task: Nullable Types\nDeclare a String variable named `username` that is allowed to be null.",
        "starterCode": "String___ username = null;",
        "solution": "String? username = null;",
        "hint": "Use a question mark ?",
        "rubric": "Uses String?."
      },
      {
        "title": "Layouts in Flutter",
        "theory": "## Flex Layouts\nTo arrange widgets horizontally, you use a `Row`. To arrange them vertically, you use a `Column`. These behave very similarly to CSS Flexbox.",
        "instructions": "## Task: Vertical Alignment\nCreate a layout widget that stacks its children vertically.",
        "starterCode": "___(\n  children: <Widget>[\n    Text('Top'),\n    Text('Bottom'),\n  ],\n)",
        "solution": "Column(\n  children: <Widget>[\n    Text('Top'),\n    Text('Bottom'),\n  ],\n)",
        "hint": "Use Column",
        "rubric": "Uses Column."
      },
      {
        ...
        "theory": "## Global State\nPassing state down through multiple widgets (prop drilling) is painful. `Provider` is the officially recommended package to inject global state into the widget tree so any widget can listen to it.",
        "instructions": "## Task: State Architecture\nWhat is the name of the popular pattern used with Provider where a model class notifies listeners of changes?",
        "starterCode": "# Options: ChangeNotifier, Redux, BLoC\npattern = '___'",
        "solution": "# Options: ChangeNotifier, Redux, BLoC\npattern = 'ChangeNotifier'",
        "hint": "ChangeNotifier",
        "rubric": "Identifies ChangeNotifier."
      },
      {
        "title": "Routing and Navigation",
        "theory": "## Managing Screens\nIn Flutter, screens are called 'Routes'. You move between screens using the `Navigator` widget, pushing new routes onto a stack.",
        "instructions": "## Task: Push a Route\nWrite the command used to push a new screen onto the navigation stack.",
        "starterCode": "Navigator.___(context, MaterialPageRoute(builder: (context) => SecondScreen()));",
        "solution": "Navigator.push(context, MaterialPageRoute(builder: (context) => SecondScreen()));",
        "hint": "Use push",
        "rubric": "Uses push."
      },
      {
        "title": "API Integration",
        "theory": "## Fetching Data\nTo fetch data from a REST API, you typically use the `http` package and dart's built-in `dart:convert` library to decode the JSON response into a Dart Map.",
        "instructions": "## Task: Decode JSON\nDecode the raw JSON string response into a dynamic map.",
        "starterCode": "import 'dart:convert';\n\nvar data = ___.decode(response.body);",
        "solution": "import 'dart:convert';\n\nvar data = json.decode(response.body);",
        "hint": "Use json.decode",
        "rubric": "Uses json.decode."
      },
      {
        "title": "Animations in Flutter",
        "theory": "## Implicit Animations\nFlutter makes animation incredibly easy with 'Implicitly Animated Widgets'. For example, if you change the properties of an `AnimatedContainer`, it will automatically transition smoothly without you writing an animation controller.",
        "instructions": "## Task: Animated Widget\nChange the standard `Container` to one that automatically animates changes to its size or color.",
        "starterCode": "___(\n  duration: Duration(seconds: 1),\n  color: isBlue ? Colors.blue : Colors.red,\n)",
        "solution": "AnimatedContainer(\n  duration: Duration(seconds: 1),\n  color: isBlue ? Colors.blue : Colors.red,\n)",
        "hint": "Use AnimatedContainer",
        "rubric": "Uses AnimatedContainer."
      },
      {
        "title": "Publishing to App Stores",
        "theory": "## The Final Build\nBefore you can upload your Flutter app to the Google Play Store or Apple App Store, you must digitally sign the app and compile it into a release bundle.",
        "instructions": "## Task: Build Command\nWhat terminal command creates an optimized release bundle for Android?",
        "starterCode": "flutter build ___",
        "solution": "flutter build appbundle",
        "hint": "Build an appbundle",
        "rubric": "Uses appbundle."
      }
    ]
  },
  "Python Basics": {
    ...
    "lessons": [
      {
        ...
        "theory": `# Variables & Data Types in Python\n\nWelcome to Python! In programming, we need a way to store information so we can use it later. We do this using **variables**. \n\nThink of a variable as a labeled box. You can put data into the box, and whenever you need that data, you just ask for the box by its label.\n\n## Creating Variables\nIn many programming languages, you have to tell the computer exactly what *kind* of box you are making before you can put something inside it. Python is dynamically typed, which means it automatically figures out what kind of box you need based on what you put inside!\n\nTo create a variable, you simply type a name, an equals sign \`=\`, and the value you want to store.\n\n\`\`\`python\nname = 'Alice' \n\`\`\`\n\n**Breaking it down:**\n- \`name\`: This is the label of our box (the variable name).\n- \`=\`: This is the **assignment operator**. It tells Python to take what's on the right and put it into the box on the left.\n- \`'Alice'\`: This is the data we are storing. Because it is wrapped in quotes, Python knows it is text.\n\n## Core Data Types\nPython has four primary data types that you will use constantly:\n\n1. **Strings (\`str\`)**: Text data. Always wrapped in single quotes \`''\` or double quotes \`""\`.\n2. **Integers (\`int\`)**: Whole numbers, positive or negative, without decimals.\n3. **Floats (\`float\`)**: Decimal numbers.\n4. **Booleans (\`bool\`)**: Represents truth values. Can only be exactly \`True\` or \`False\` (capitalized!).\n\nLet's look at examples of each:\n\n\`\`\`python\n# 1. String (Text)\ncharacter_name = "Jon Snow"\n\n# 2. Integer (Whole Number)\nlevel = 42\n\n# 3. Float (Decimal Number)\nhealth_points = 99.5\n\n# 4. Boolean (True or False)\nis_alive = True\n\`\`\`\n> **Tip:** The \`#\` symbol creates a **comment**. Python completely ignores everything after the \`#\`. We use comments to leave notes for ourselves and other programmers!\n\n## The \`type()\` Function\nIf you ever forget what is inside a box, you can use Python's built-in \`type()\` function to check.\n\n\`\`\`python\nmystery_variable = 3.14\nprint(type(mystery_variable)) \n# Output: <class 'float'>\n\`\`\`\n\n## Formatted Strings (f-strings)\nWhen we want to print variables out in a sentence, the most modern and readable way is using **f-strings** (formatted strings). Just put an \`f\` before the quotes, and wrap your variables in curly braces \`{}\`!\n\n\`\`\`python\napples = 5\nprint(f'I have {apples} apples in my basket.')\n# Output: I have 5 apples in my basket.\n\`\`\`\n\nNow it's your turn. Head over to the instructions tab to create your own variables!`,
      ...
      },
      {
        ...
        "theory": `# String Manipulation & Operations\n\nIn Python, text is referred to as a **String** (short for a string of characters). Because text processing is such a massive part of programming (think about search engines, chatbots, and data parsing), Python gives us incredibly powerful tools—called **methods**—to manipulate strings easily.\n\n## What is a Method?\nA method is essentially a built-in command that belongs to a specific data type. You use a method by typing the variable name, a dot \`.\`, and the method name followed by parentheses \`()\$.\n\nLet's look at some of the most common string methods:\n\n\`\`\`python\nmessage = "python is amazing"\n\n# .upper() converts everything to uppercase\nprint(message.upper())  \n# Output: PYTHON IS AMAZING\n\n# .title() capitalizes the first letter of every word\nprint(message.title())  \n# Output: Python Is Amazing\n\`\`\`\n\n## Checking String Length\nSometimes we need to know exactly how many characters are in a string (for example, checking if a password is long enough). We use the built-in \`len()\` function.\n\n\`\`\`python\npassword = "supersecret"\nlength = len(password)\nprint(f"Your password is {length} characters long.")\n# Output: Your password is 11 characters long.\n\`\`\`\n> **Note:** \`len()\` counts *everything* inside the quotes, including spaces and punctuation!\n\n## String Slicing (Extracting Substrings)\nWhat if we only want a specific part of a string? We can extract chunks of text using a technique called **slicing**. \n\nIn Python, we count characters starting from **zero**. \n- The 1st character is at index \`0\`\n- The 2nd character is at index \`1\`\n\nTo slice a string, use square brackets with a start and stop index: \`[start:stop]\`. The slice includes the start index, but goes *up to, but not including* the stop index.\n\n\`\`\`python\nword = "DATACAMP"\n# D A T A C A M P\n# 0 1 2 3 4 5 6 7\n\n# Get characters from index 0 up to (but not including) 4\nprint(word[0:4])  \n# Output: DATA\n\n# Get characters from index 4 up to 8\nprint(word[4:8])  \n# Output: CAMP\n\`\`\`\n\nNow, try manipulating strings yourself in the exercise!`,
    ...
    "lessons": [
      {
        ...
        "theory": "## Making Decisions in Code\nConditional statements let your program make decisions based on conditions.\n\n```python\nscore = 85\n\nif score >= 90:\n    grade = 'A'\nelif score >= 80:\n    grade = 'B'\nelif score >= 70:\n    grade = 'C'\nelse:\n    grade = 'F'\n\nprint(f'Your grade: {grade}')  # B\n```\n\nPython uses **indentation** (4 spaces) to define code blocks.\n\n**What this code does:**\n- `if score >= 90:`: Checks if the condition is True. If it is, it runs the indented code directly below it.\n- `elif score >= 80:`: Stands for 'else if'. If the first `if` was False, check this condition next.\n- `else:`: The catch-all. If *none* of the conditions above were True, run this block of code.\n- Notice the indentation! Python relies on those 4 spaces to know which code belongs inside the `if` block.",
      ...
      {
        "title": "For Loops",
        "theory": "## Repeating Actions with For Loops\nFor loops iterate over a sequence (list, string, range).\n\n```python\n# Loop through a list\nfruits = ['apple', 'banana', 'cherry']\nfor fruit in fruits:\n    print(fruit)\n\n# Loop with range\nfor i in range(5):    # 0, 1, 2, 3, 4\n    print(i)\n\n# Range with start, stop, step\nfor i in range(2, 10, 2):  # 2, 4, 6, 8\n    print(i)\n```\n\n**What this code does:**\n- `for fruit in fruits:`: A loop that says 'go through the `fruits` list one by one. For each item, temporarily call it `fruit` and run the indented code'.\n- `range(5)`: A built-in function that generates a sequence of numbers from 0 up to (but not including) 5.\n- `range(2, 10, 2)`: Generates numbers starting at 2, stopping before 10, stepping by 2 each time.",
    ...
    "lessons": [
      {
        ...
        "theory": "## Functions: Reusable Code Blocks\nFunctions let you write code once and reuse it many times.\n\n```python\ndef greet(name):\n    return f'Hello, {name}!'\n\nmessage = greet('Alice')\nprint(message)  # Hello, Alice!\n\n# Default parameters\ndef power(base, exp=2):\n    return base ** exp\n\nprint(power(3))     # 9\nprint(power(3, 3))  # 27\n```\n\n**What this code does:**\n- `def greet(name):`: `def` defines a new function. `greet` is the name. `name` is the parameter (input) it expects.\n- `return ...`: The function evaluates the f-string and spits that value back out to the caller.\n- `message = greet('Alice')`: We call the function, pass 'Alice' into it, and store the returned result in `message`.\n- `exp=2`: A default parameter. If the caller doesn't provide a second number, the function assumes it's 2.",
    ...
    "lessons": [
      {
        ...
        "instructions": "## Task: Use an F-String\nFormat the string to say 'My name is Alice and I am 30 years old.' using the variables.",
        "starterCode": "name = 'Alice'\nage = 30\ngreeting = ___'My name is {___} and I am {___} years old.'",
        "solution": "name = 'Alice'\nage = 30\ngreeting = f'My name is {name} and I am {age} years old.'",
        "hint": "Use the f prefix and the variable names",
        "rubric": "Correctly uses f-strings with {name} and {age}."
      },
      {
        "title": "Splitting and Joining",
        "theory": "## String to List and Back\nThe `.split()` method breaks a string into a list based on a delimiter, and `.join()` merges a list of strings into one.",
        "instructions": "## Task: Join Words\nJoin the list of words into a single string separated by spaces.",
        "starterCode": "words = ['Hello', 'world']\nsentence = '___'.___(words)",
        "solution": "words = ['Hello', 'world']\nsentence = ' '.join(words)",
        "hint": "Use ' '.join()",
        "rubric": "Uses ' '.join(words)."
      }
    ]
  },
  "Error Handling": {
    ...
    "lessons": [
      {
        ...
        "theory": "## Catching Exceptions\nTo prevent your program from crashing when an error occurs, you can wrap risky code in a `try` block and handle the error in an `except` block.",
        "instructions": "## Task: Catch Division by Zero\nCatch the specific error that occurs when dividing by zero.",
        "starterCode": "try:\n    result = 10 / 0\nexcept ___:\n    print(\"Cannot divide by zero!\")",
        "solution": "try:\n    result = 10 / 0\nexcept ZeroDivisionError:\n    print(\"Cannot divide by zero!\")",
        "hint": "The error is ZeroDivisionError",
        "rubric": "Uses ZeroDivisionError."
      },
      {
        "title": "Finally Clause",
        "theory": "## Guaranteed Execution\nThe `finally` block executes regardless of whether an exception was thrown or caught. It's often used for cleanup, like closing files.",
        "instructions": "## Task: Add Finally\nAdd the block that ensures 'Cleanup done.' is always printed.",
        "starterCode": "try:\n    pass\nexcept Exception:\n    pass\n___:\n    print(\"Cleanup done.\")",
        "solution": "try:\n    pass\nexcept Exception:\n    pass\nfinally:\n    print(\"Cleanup done.\")",
        "hint": "Use the finally keyword",
        "rubric": "Uses the finally block."
      }
    ]
  },
  "List Comprehensions": {
    ...
    "lessons": [
      {
        "title": "Compact Loops",
        "theory": "## Pythonic Lists\nA list comprehension provides a shorter syntax to create a new list based on the values of an existing list.",
        "instructions": "## Task: Square the Numbers\nUse a list comprehension to create a list of squares for `x` in `nums`.",
        "starterCode": "nums = [1, 2, 3]\nsquares = [___ for ___ in ___]",
        "solution": "nums = [1, 2, 3]\nsquares = [x**2 for x in nums]",
        "hint": "Use x**2 for x in nums",
        "rubric": "Correctly writes x**2 for x in nums."
      },
      {
        "title": "Adding Conditions",
        "theory": "## Filtering Lists\nYou can add an `if` statement to the end of a list comprehension to filter items.",
        "instructions": "## Task: Filter Evens\nCreate a list of only the even numbers from `nums`.",
        "starterCode": "nums = [1, 2, 3, 4]\nevens = [x for x in nums if x ___ 2 == ___]",
        "solution": "nums = [1, 2, 3, 4]\nevens = [x for x in nums if x % 2 == 0]",
        "hint": "Use x % 2 == 0",
        "rubric": "Correctly uses x % 2 == 0."
      }
    ]
  },
  "Context Managers": {
    ...
    "lessons": [
      {
        ...
        "theory": "## Automatic Resource Management\nThe `with` statement in Python is used with context managers to automatically acquire and release resources, ensuring files are closed even if errors occur.",
        "instructions": "## Task: Open a File\nUse the `with` statement to open 'data.txt' in read mode.",
        "starterCode": "___ open('data.txt', 'r') ___ f:\n    content = f.read()",
        "solution": "with open('data.txt', 'r') as f:\n    content = f.read()",
        "hint": "Use with ... as f:",
        "rubric": "Correctly uses with and as."
      },
      {
        "title": "Custom Context Managers",
        "theory": "## __enter__ and __exit__\nTo create a custom class that works with the `with` statement, you must implement the `__enter__` and `__exit__` dunder methods.",
        "instructions": "## Task: Define Enter Method\nDefine the magic method that is called when entering the `with` block.",
        "starterCode": "class Timer:\n    def ___(self):\n        self.start = time.time()\n        return self",
        "solution": "class Timer:\n    def __enter__(self):\n        self.start = time.time()\n        return self",
        "hint": "The method is __enter__",
        "rubric": "Defines __enter__ method."
      }
    ]
  },
  "Concurrency & Asyncio": {
    ...
    "lessons": [
      {
        "title": "Async/Await",
        "theory": "## Asynchronous I/O\n`asyncio` allows you to write concurrent code using the `async` and `await` syntax. It is perfect for IO-bound tasks like network requests.",
        "instructions": "## Task: Define Async Function\nDefine a coroutine function named `fetch_data`.",
        "starterCode": "___ def fetch_data():\n    pass",
        "solution": "async def fetch_data():\n    pass",
        "hint": "Use the async keyword",
        "rubric": "Correctly uses the async keyword."
      },
      {
        "title": "Awaiting Coroutines",
        "theory": "## Yielding Control\nWhen you use `await`, the function yields control back to the event loop until the awaited task is complete, allowing other tasks to run.",
        "instructions": "## Task: Await Sleep\nPause the coroutine for 1 second without blocking the entire thread.",
        "starterCode": "import asyncio\n\nasync def main():\n    ___ asyncio.sleep(1)",
        "solution": "import asyncio\n\nasync def main():\n    await asyncio.sleep(1)",
        "hint": "Use the await keyword",
        "rubric": "Correctly uses the await keyword."
      }
    ]
  },
  "Dictionaries & Sets": {
    ...
    "lessons": [
      {
        ...
        "theory": "## Dictionaries\nA dictionary (`dict`) stores data in key-value pairs. Keys must be unique and immutable (like strings or numbers).",
        "instructions": "## Task: Accessing Values\nRetrieve the value associated with the key 'age' from the dictionary.",
        "starterCode": "person = {'name': 'Alice', 'age': 30}\nuser_age = person['___']",
        "solution": "person = {'name': 'Alice', 'age': 30}\nuser_age = person['age']",
        "hint": "Use 'age' as the key",
        "rubric": "Correctly accesses the 'age' key."
      },
      {
        "title": "Unique Elements",
        "theory": "## Sets\nA set is an unordered collection of unique elements. It is highly optimized for checking if an item exists within it.",
        "instructions": "## Task: Remove Duplicates\nConvert the list to a set to automatically remove duplicate values.",
        "starterCode": "nums = [1, 2, 2, 3, 3, 3]\nunique_nums = ___(nums)",
        "solution": "nums = [1, 2, 2, 3, 3, 3]\nunique_nums = set(nums)",
        "hint": "Use the set() constructor",
        "rubric": "Uses set(nums)."
      }
    ]
  },
  "Lambda Functions & Map/Filter": {
    ...
    "lessons": [
      {
        ...
        "instructions": "## Task: Write a Lambda\nWrite a lambda function that multiplies its argument `x` by 10.",
        "starterCode": "multiply_by_10 = ___ x: x ___ 10",
        "solution": "multiply_by_10 = lambda x: x * 10",
        "hint": "Use lambda and *",
        "rubric": "Correctly defines the lambda x: x * 10."
      },
      {
        "title": "Map and Filter",
        "theory": "## Functional Tools\n`map()` applies a function to all items in an iterable. `filter()` creates a list of elements for which a function returns True.",
        "instructions": "## Task: Map a Function\nUse `map` to apply the `str` function to every integer in the list.",
        "starterCode": "nums = [1, 2, 3]\nstr_nums = list(map(___, nums))",
        "solution": "nums = [1, 2, 3]\nstr_nums = list(map(str, nums))",
        "hint": "Pass the 'str' function to map",
        "rubric": "Uses map(str, nums)."
      }
    ]
  },
  "Regular Expressions in Python": {
    ...
    "lessons": [
      {
        "title": "The RE Module",
        "theory": "## Pattern Matching\nPython's built-in `re` module allows you to search, replace, and parse strings using Regular Expressions.",
        "instructions": "## Task: Find All\nUse `re.findall()` to find all occurrences of digits (`\\d+`) in the string.",
        "starterCode": "import re\n\ntext = 'I have 2 apples and 10 oranges.'\nnumbers = re.findall(r'___', text)",
        "solution": "import re\n\ntext = 'I have 2 apples and 10 oranges.'\nnumbers = re.findall(r'\\d+', text)",
        "hint": "Use \\d+",
        "rubric": "Correctly uses \\d+ to match digits."
      },
      {
        "title": "Regex Substitution",
        "theory": "## Search and Replace\nThe `re.sub()` function replaces the matches in a string with a replacement string.",
        "instructions": "## Task: Redact Info\nUse `re.sub()` to replace any sequence of digits with the string '[REDACTED]'.",
        "starterCode": "import re\n\ntext = 'My pin is 1234'\nredacted = re.sub(r'\\d+', '___', text)",
        "solution": "import re\n\ntext = 'My pin is 1234'\nredacted = re.sub(r'\\d+', '[REDACTED]', text)",
        "hint": "Use '[REDACTED]'",
        "rubric": "Replaces with '[REDACTED]'."
      }
    ]
  },
  "Metaclasses": {
    ...
    "lessons": [
      {
        ...
        "theory": "## The Class Factory\nIn Python, a class is just an object that creates instances. A Metaclass is the 'class of a class'—it is the factory that creates class objects.",
        "instructions": "## Task: Identify the Base\nWhat is the built-in metaclass that all default Python classes are instances of?",
        "starterCode": "# Options: object, type, class\ndefault_metaclass = '___'",
        "solution": "# Options: object, type, class\ndefault_metaclass = 'type'",
        "hint": "The answer is 'type'",
        "rubric": "Identifies 'type'."
      },
      {
        "title": "Custom Metaclasses",
        "theory": "## Hooking into Class Creation\nBy overriding `__new__` in a metaclass, you can automatically alter the attributes or methods of a class at the exact moment it is defined.",
        "instructions": "## Task: Metaclass Keyword\nSpecify `MyMeta` as the metaclass for the `MyClass` definition.",
        "starterCode": "class MyClass(___ = MyMeta):\n    pass",
        "solution": "class MyClass(metaclass = MyMeta):\n    pass",
        "hint": "Use the 'metaclass' keyword argument",
        "rubric": "Uses metaclass=MyMeta."
      }
    ]
  },
  "Type Hinting & Pydantic": {
    ...
    "lessons": [
      {
        "title": "Static Typing",
        "theory": "## Hints, not Rules\nPython 3.5+ introduced type hints. They don't enforce types at runtime, but they allow IDEs and tools like `mypy` to catch bugs before you run the code.",
        "instructions": "## Task: Hint a Return Type\nAdd a type hint indicating that the function `greet` returns a `str`.",
        "starterCode": "def greet(name: str) -> ___:\n    return f'Hello {name}'",
        "solution": "def greet(name: str) -> str:\n    return f'Hello {name}'",
        "hint": "Use -> str:",
        "rubric": "Correctly sets the return type to str."
      },
      {
        "title": "Pydantic Models",
        "theory": "## Runtime Validation\nUnlike standard type hints, the `pydantic` library uses type hints to perform *strict runtime validation* and data parsing. It is heavily used in FastAPI.",
        "instructions": "## Task: Base Model\nMake the `User` class inherit from Pydantic's `BaseModel`.",
        "starterCode": "from pydantic import BaseModel\n\nclass User(___):\n    id: int\n    name: str",
        "solution": "from pydantic import BaseModel\n\nclass User(BaseModel):\n    id: int\n    name: str",
        "hint": "Inherit from BaseModel",
        "rubric": "Inherits from BaseModel."
      }
    ]
  },
  "Advanced OOP & Magic Methods": {
    ...
    "lessons": [
      {
        "title": "Instantiation vs Initialization",
        "theory": "## Object Creation\nMost developers only use `__init__` (Initialization). But `__new__` is the method actually responsible for *Creating* (Instantiating) the object before `__init__` is ever called.",
        "instructions": "## Task: The True Constructor\nWhat magic method is the true constructor of a class and must return an instance?",
        "starterCode": "true_constructor = '___'",
        "solution": "true_constructor = '__new__'",
        "hint": "It's __new__",
        "rubric": "Identifies __new__."
      },
      {
        "title": "Str vs Repr",
        "theory": "## String Representation\n`__str__` is meant to be readable for end-users, while `__repr__` is meant to be unambiguous for developers (ideally, it should return a string that could recreate the object).",
        "instructions": "## Task: Implement Repr\nImplement the `__repr__` method to return 'Point(x, y)'.",
        "starterCode": "class Point:\n    def __init__(self, x, y):\n        self.x, self.y = x, y\n    def ___:\n        return f'Point({self.x}, {self.y})'",
        "solution": "class Point:\n    def __init__(self, x, y):\n        self.x, self.y = x, y\n    def __repr__(self):\n        return f'Point({self.x}, {self.y})'",
        "hint": "Define def __repr__(self):",
        "rubric": "Defines __repr__."
      },
      {
        "title": "Equality and Hashing",
        "theory": "## Custom Comparisons\nBy default, objects are compared by memory address. Overriding `__eq__` lets you compare them by value. If you override `__eq__` and want the object to be used as a dictionary key, you MUST also override `__hash__`.",
        "instructions": "## Task: Override Equality\nOverride the method that is called when you do `obj1 == obj2`.",
        "starterCode": "class Item:\n    def ___:\n        return self.id == other.id",
        "solution": "class Item:\n    def __eq__(self, other):\n        return self.id == other.id",
        "hint": "Define def __eq__(self, other):",
        "rubric": "Defines __eq__(self, other)."
      },
      {
        "title": "Property Decorators",
        "theory": "## Managed Attributes\nThe `@property` decorator allows you to define methods that can be accessed like attributes, giving you getter/setter control without breaking the API.",
        "instructions": "## Task: Make a Getter\nTurn the `temperature` method into a read-only property.",
        "starterCode": "class Sensor:\n    ___\n    def temperature(self):\n        return 72",
        "solution": "class Sensor:\n    @property\n    def temperature(self):\n        return 72",
        "hint": "Use @property",
        "rubric": "Uses @property decorator."
      },
      {
        "title": "Classmethods vs Staticmethods",
        "theory": "## Method Binding\nInstance methods take `self`. `@classmethod` takes `cls` and is often used for alternative constructors. `@staticmethod` takes neither and is just a regular function living in a class namespace.",
        "instructions": "## Task: Alternative Constructor\nUse the correct decorator to create a factory method that takes the class `cls` as its first argument.",
        "starterCode": "class User:\n    ___\n    def from_string(cls, data):\n        return cls(data)",
        "solution": "class User:\n    @classmethod\n    def from_string(cls, data):\n        return cls(data)",
        "hint": "Use @classmethod",
        "rubric": "Uses @classmethod."
      },
      {
        "title": "Multiple Inheritance & MRO",
        "theory": "## Method Resolution Order\nWhen inheriting from multiple classes, Python uses C3 Linearization to determine the Method Resolution Order (MRO). You can view this order by checking `ClassName.__mro__`.",
        "instructions": "## Task: Check the MRO\nAccess the built-in attribute that lists the class search path for method resolution.",
        "starterCode": "mro_tuple = MyClass.___",
        "solution": "mro_tuple = MyClass.__mro__",
        "hint": "Access __mro__",
        "rubric": "Uses __mro__."
      },
      {
        "title": "Callable Instances",
        "theory": "## Objects as Functions\nBy implementing the `__call__` magic method, you can execute an instance of a class exactly as if it were a function.",
        "instructions": "## Task: Make it Callable\nDefine the magic method so that calling `counter()` executes it.",
        "starterCode": "class Counter:\n    def ___:\n        self.count += 1\n\ncounter = Counter()\ncounter()",
        "solution": "class Counter:\n    def __call__(self):\n        self.count += 1\n\ncounter = Counter()\ncounter()",
        "hint": "Define def __call__(self):",
        "rubric": "Defines __call__."
      },
      {
        "title": "Indexing and Slicing",
        "theory": "## Behaving like a List\nIf you want your custom object to support indexing like `my_obj[0]` or dictionary lookups like `my_obj['key']`, you implement `__getitem__` and `__setitem__`.",
        "instructions": "## Task: Implement Get Item\nDefine the magic method that is triggered by bracket notation.",
        "starterCode": "class CustomList:\n    def ___:\n        return self.data[index]",
        "solution": "class CustomList:\n    def __getitem__(self, index):\n        return self.data[index]",
        "hint": "Define def __getitem__(self, index):",
        "rubric": "Defines __getitem__."
      }
    ]
  },
  "Data Classes & Modern Python Masterclass": {
    ...
    "lessons": [
      {
        "title": "Introduction to Data Classes",
        "theory": "## Less Boilerplate\nIntroduced in Python 3.7, `@dataclass` automatically generates special methods like `__init__`, `__repr__`, and `__eq__` for classes that primarily store data.",
        "instructions": "## Task: The Decorator\nApply the decorator from the `dataclasses` module to automatically generate an `__init__` method for the `User` class.",
        "starterCode": "from dataclasses import dataclass\n\n___\nclass User:\n    name: str\n    age: int",
        "solution": "from dataclasses import dataclass\n\n@dataclass\nclass User:\n    name: str\n    age: int",
        "hint": "Use @dataclass",
        "rubric": "Uses @dataclass."
      },
      {
        "title": "Default Factories",
        "theory": "## Mutable Defaults\nIn Python, you should never use a mutable object (like an empty list) as a default argument. In dataclasses, you solve this by using `field(default_factory=list)` to create a fresh list for every instance.",
        "instructions": "## Task: Safe Defaults\nUse `field` and `default_factory` to safely initialize `items` as an empty list.",
        "starterCode": "from dataclasses import dataclass, field\n\n@dataclass\nclass Cart:\n    items: list = ___(default_factory=___)",
        "solution": "from dataclasses import dataclass, field\n\n@dataclass\nclass Cart:\n    items: list = field(default_factory=list)",
        "hint": "Use field and list",
        "rubric": "Uses field(default_factory=list)."
      },
      {
        "title": "Post-Init Processing",
        "theory": "## After Initialization\nBecause `@dataclass` generates `__init__` for you, you can't put custom validation logic in it. Instead, dataclasses provide a `__post_init__` method that runs immediately after the generated `__init__`.",
        "instructions": "## Task: Custom Validation\nDefine the magic method that dataclasses call right after initialization.",
        "starterCode": "@dataclass\nclass Person:\n    age: int\n\n    def ___:\n        if self.age < 0:\n            raise ValueError('Age cannot be negative')",
        "solution": "@dataclass\nclass Person:\n    age: int\n\n    def __post_init__(self):\n        if self.age < 0:\n            raise ValueError('Age cannot be negative')",
        "hint": "Define def __post_init__(self):",
        "rubric": "Defines __post_init__."
      },
      {
        "title": "The Walrus Operator",
        "theory": "## Assignment Expressions\nIntroduced in Python 3.8, the walrus operator `:=` allows you to assign a value to a variable and return that value in the same expression. This is very useful in `if` and `while` loops.",
        "instructions": "## Task: Assign and Check\nUse the walrus operator to assign the result of `get_data()` to `data` and check if it is truthy in one line.",
        "starterCode": "if (data ___ get_data()):\n    print(f'Received: {data}')",
        "solution": "if (data := get_data()):\n    print(f'Received: {data}')",
        "hint": "Use :=",
        "rubric": "Uses :=."
      },
      {
        "title": "Structural Pattern Matching",
        "theory": "## Match / Case\nIntroduced in Python 3.10, `match / case` acts like a supercharged switch statement. It doesn't just match values; it can match structures, types, and unpack data.",
        "instructions": "## Task: Pattern Matching\nUse the correct keyword to match the `status` variable against cases.",
      ...
      },
      {
        ...
        "theory": "## Self-Documenting Strings\nIn Python 3.8, f-strings got a powerful debugging feature. Adding an equals sign `=` to the end of a variable inside an f-string will print both the variable name and its value.",
        "instructions": "## Task: Debug Print\nUse the f-string debugging feature so that it prints 'user_id=42'.",
        "starterCode": "user_id = 42\nprint(f'{user_id___}')",
        "solution": "user_id = 42\nprint(f'{user_id=}')",
        "hint": "Add an = sign",
        "rubric": "Adds =."
      },
      {
        "title": "Dictionary Union Operators",
        "theory": "## Merging Dicts Cleanly\nIn Python 3.9, you can merge dictionaries using the `|` operator, and update them using the `|=` operator, similar to how sets work. If there are duplicate keys, the right-side dictionary wins.",
        "instructions": "## Task: Merge Dictionaries\nUse the pipe operator to merge `dict1` and `dict2` into a new dictionary.",
        "starterCode": "dict1 = {'a': 1, 'b': 2}\ndict2 = {'b': 3, 'c': 4}\nmerged = dict1 ___ dict2",
      ...
      },
      {
        ...
        "theory": "## Enforcing API Design\nIn Python 3.8, the `/` syntax was introduced in function signatures to indicate that some parameters *must* be passed positionally and cannot be passed as keyword arguments.",
        "instructions": "## Task: Positional Only\nPlace the correct symbol to enforce that `x` and `y` must be positional, but `z` can be a keyword argument.",
        "starterCode": "def power(x, y, ___, z=1):\n    return (x ** y) * z",
        "solution": "def power(x, y, /, z=1):\n    return (x ** y) * z",
        "hint": "Use the / symbol",
        "rubric": "Uses /."
      }
    ]
